...
void ImpliedValue::getImpliedValues(ref<Expr> e,
  ...
  case Expr::Select: {
    ...
    if (ConstantExpr *TrueCE = dyn_cast<ConstantExpr>(se->getTrue())) {
      if (ConstantExpr *FalseCE = dyn_cast<ConstantExpr>(se->getFalse())) {
        if (TrueCE != FalseCE) {
          if (value == TrueCE) {
            getImpliedValues(se->getCondition(), ConstantExpr::alloc(1, Expr::Bool),
                             results);
          } else {
            assert(value == FalseCE &&
                   "err in implied value calculation");
            getImpliedValues(se->getCondition(), ConstantExpr::alloc(0, Expr::Bool),
                             results);
          }
        }
      }
  ...
  case Expr::SExt: {
    ...
    getImpliedValues(ce->getSrc(), value->Extract(0, ce->getSrc()->getWidth()),
                     results);
    break;
  }
  ...
  case Expr::Add: { // constants on left
    ...
    if (ConstantExpr *CE = dyn_cast<ConstantExpr>(be->getLeft()))
      getImpliedValues(be->getRight(), value->Sub(CE), results);
    break;
  }
  case Expr::Sub: { // constants on left
    ...
    // C_0 - A = C  =>  A = C_0 - C
    if (ConstantExpr *CE = dyn_cast<ConstantExpr>(be->getLeft()))
      getImpliedValues(be->getRight(), CE->Sub(value), results);
    break;
  }
  case Expr::Mul: {
  ...
  case Expr::And: {
    ...
    if (be->getWidth() == Expr::Bool) {
      if (value->isTrue()) {
        getImpliedValues(be->getLeft(), value, results);
        getImpliedValues(be->getRight(), value, results);
      }
    } else {
  ...
  case Expr::Or: {
    BinaryExpr *be = cast<BinaryExpr>(e);
    if (value->isZero()) {
      getImpliedValues(be->getLeft(), 0, results);
      getImpliedValues(be->getRight(), 0, results);
    } else {
      // FIXME: Can do more?
    }
    break;
  ...
  case Expr::Xor: {
    BinaryExpr *be = cast<BinaryExpr>(e);
    if (ConstantExpr *CE = dyn_cast<ConstantExpr>(be->getLeft()))
      getImpliedValues(be->getRight(), value->Xor(CE), results);
    break;
  }
  ...
  case Expr::Eq: {
    ...
    if (value->isTrue()) {
      if (ConstantExpr *CE = dyn_cast<ConstantExpr>(ee->getLeft()))
        getImpliedValues(ee->getRight(), CE, results);
    } else {
      ...
      if (ConstantExpr *CE = dyn_cast<ConstantExpr>(ee->getLeft()))
        if (CE->getWidth() == Expr::Bool)
          getImpliedValues(ee->getRight(), CE->Not(), results);
    }
    break;
  }
...
void ImpliedValue::checkForImpliedValues(Solver *S, ref<Expr> e,
  ...
  for (std::vector< ref<ReadExpr> >::iterator i = reads.begin(),
         ie = reads.end(); i != ie; ++i) {
    ReadExpr *re = i->get();
    assumption.push_back(UltExpr::create(re->getIndex(),
                                         ConstantExpr::alloc(re->getUpdates().getRoot()->getSize(),
