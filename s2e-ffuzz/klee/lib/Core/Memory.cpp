...
void MemoryObject::split(std::vector<MemoryObject*> &objects,
...
ObjectState::ObjectState(): concreteMask(0),
    copyOnWriteOwner(0),
    refCount(0),
    object(NULL),
    concreteStore(NULL),
...
ObjectState::ObjectState(const MemoryObject *mo)
  : concreteMask(0),
    ...
    refCount(0),
    object(mo),
    concreteStore(new ConcreteBuffer(mo->size)),
    storeOffset(0),
    flushMask(0),
    knownSymbolics(0),
    updates(0, 0),
...
ObjectState::ObjectState(const ObjectState &os)
  : concreteMask(os.concreteMask ? new BitArray(*os.concreteMask, os.size) : 0),
    ...
    refCount(0),
    object(os.object),
    concreteStore(new ConcreteBuffer(os.size)),
    storeOffset(0),
...
ObjectState::~ObjectState() {
  if (concreteMask) concreteMask->decref();
  if (flushMask) flushMask->decref();
...
const UpdateList &ObjectState::getUpdates() const {
  ...
  if (!updates.getRoot()) {
    ...
    const Array *array = new Array("const_arr" + llvm::utostr(++const_arr_id), size,
                                   &Contents[0],
