...
public:
    ...
    CexObjectData &getObjectData(const Array *A) {
        ...
        if (!Entry)
            Entry = new CexObjectData(A->getSize());
    ...
    void propogatePossibleValues(ref<Expr> e, CexValueData range) {
        ...
        case Expr::Select: {
            ...
            ValueRange cond = evalRangeForExpr(se->getCondition());
            if (cond.isFixed()) {
                if (cond.min()) {
                    propogatePossibleValues(se->getTrue(), range);
                } else {
                    propogatePossibleValues(se->getFalse(), range);
                }
            } else {
        ...
        case Expr::ZExt: {
            ...
            unsigned inBits = ce->getSrc()->getWidth();
            ValueRange input =
                    range.set_intersection(ValueRange(0, bits64::maxValueOfNBits(inBits)));
            propogatePossibleValues(ce->getSrc(), input);
            break;
        }
        ...
        case Expr::Add: {
            ...
            if (ConstantExpr *CE = dyn_cast<ConstantExpr>(be->getLeft())) {
                ...
                if (CE->getWidth() <= 64) {
                    ...
                    if (!nrange.isEmpty())
                        propogatePossibleValues(be->getRight(), nrange);
                }
            }
            break;
        }
        ...
        case Expr::And: {
            ...
            if (be->getWidth()==Expr::Bool) {
                if (range.isFixed()) {
                    ValueRange left = evalRangeForExpr(be->getLeft());
                    ValueRange right = evalRangeForExpr(be->getRight());
                    ...
                    if (!range.min()) {
                        if (left.mustEqual(0) || right.mustEqual(0)) {
                            ...
                            propogatePossibleValue(be->getLeft(), 0);
                            left = evalRangeForExpr(be->getLeft());

                            // see if that worked
                            if (!left.mustEqual(1))
                                propogatePossibleValue(be->getRight(), 0);
                        }
                    } else {
                        if (!left.mustEqual(1)) propogatePossibleValue(be->getLeft(), 1);
                        if (!right.mustEqual(1)) propogatePossibleValue(be->getRight(), 1);
                    }
                }
            } else {
                // XXX
                ...
                if (range.isFixed()) {
                    ...
                    if (range.min()) {
                        if (left.mustEqual(1) || right.mustEqual(1)) {
                            ...
                            propogatePossibleValue(be->getLeft(), 1);
                            left = evalRangeForExpr(be->getLeft());

                            // see if that worked
                            if (!left.mustEqual(1))
                                propogatePossibleValue(be->getRight(), 1);
                        }
                    } else {
                        if (!left.mustEqual(0)) propogatePossibleValue(be->getLeft(), 0);
                        if (!right.mustEqual(0)) propogatePossibleValue(be->getRight(), 0);
                    }
                }
            } else {
                // XXX
            ...
            if (range.isFixed()) {
                if (ConstantExpr *CE = dyn_cast<ConstantExpr>(be->getLeft())) {
                    ...
                    if (CE->getWidth() <= 64) {
                        ...
                        if (range.min()) {
                            propogatePossibleValue(be->getRight(), value);
                        } else {
                            CexValueData range;
                            if (value==0) {
                            ...
                            propogatePossibleValues(be->getRight(), range);
                        }
                    } else {
                        // XXX what now
                    }
        ...
        case Expr::Ult: {
            ...
            if (range.isFixed()) {
                ...
                uint64_t maxValue = bits64::maxValueOfNBits(be->getRight()->getWidth());

                // XXX should deal with overflow (can lead to empty range)

                if (left.isFixed()) {
                    if (range.min()) {
                        propogatePossibleValues(be->getRight(), CexValueData(left.min()+1,
                                                                        maxValue));
                    } else {
                        propogatePossibleValues(be->getRight(), CexValueData(0, left.min()));
                    }
                } else if (right.isFixed()) {
                    if (range.min()) {
                        propogatePossibleValues(be->getLeft(), CexValueData(0, right.min()-1));
                    } else {
                        propogatePossibleValues(be->getLeft(), CexValueData(right.min(),
                                                                       maxValue));
                    }
                } else {
                    // XXX ???
        ...
        case Expr::Ule: {
            ...
            if (range.isFixed()) {
                ...
                if (left.isFixed()) {
                    if (range.min()) {
                        propogatePossibleValues(be->getRight(), CexValueData(left.min(),
                                                                        maxValue));
                    } else {
                        propogatePossibleValues(be->getRight(), CexValueData(0, left.min()-1));
                    }
                } else if (right.isFixed()) {
                    if (range.min()) {
                        propogatePossibleValues(be->getLeft(), CexValueData(0, right.min()));
                    } else {
                        propogatePossibleValues(be->getLeft(), CexValueData(right.min()+1,
                                                                       maxValue));
                    }
                } else {
                    // XXX ???
    ...
    void propogateExactValues(ref<Expr> e, CexValueData range) {
        ...
        case Expr::Read: {
            ...
            for (const UpdateNode *un = re->getUpdates().getHead(); un; un = un->getNext()) {
                ...
                if (ui.mustEqual(index) || re->getIndex() == un->getIndex())
                    propogateExactValues(un->getValue(), range);
                return;
            }
            ...
            if (index.isFixed()) {
                if (array->isConstantArray()) {
                    // Verify the range.
                    propogateExactValues(array->getConstantValues()[index.min()],
                            range);
                } else {
...
static bool propogateValues(const Query& query, CexData &cd,
    ...
    for (ConstraintManager::const_iterator it = query.constraints.begin(),
        ...
        cd.propogatePossibleValue(query.constraints.toExpr(*it), 1);
        cd.propogateExactValue(query.constraints.toExpr(*it), 1);
    }
    if (checkExpr) {
    ...
    for (ConstraintManager::const_iterator it = query.constraints.begin(),
         ie = query.constraints.end(); it != ie; ++it) {
        if (hasSatisfyingAssignment && !cd.evaluatePossible(query.constraints.toExpr(*it))->isTrue())
            hasSatisfyingAssignment = false;

        // If this constraint is known to be false, then we can prove anything, so
        // the query is valid.
        if (cd.evaluateExact(query.constraints.toExpr(*it))->isFalse()) {
            isValid = true;
            return true;
        }
    }
