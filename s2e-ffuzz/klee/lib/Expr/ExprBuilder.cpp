...
public:
    ...
    ref<Expr> Add(const ref<ConstantExpr> &LHS,
        ...
        case Expr::Add: {
            ...
            if (ConstantExpr *CE = dyn_cast<ConstantExpr>(BE->getLeft()))
                return Builder->Add(LHS->Add(CE), BE->getRight());
            // C_0 + (X + C_1) ==> (C_0 + C1) + X
            if (ConstantExpr *CE = dyn_cast<ConstantExpr>(BE->getRight()))
                return Builder->Add(LHS->Add(CE), BE->getLeft());
            break;
        }
        ...
        case Expr::Sub: {
            ...
            // C_0 + (C_1 - X) ==> (C_0 + C1) - X
            if (ConstantExpr *CE = dyn_cast<ConstantExpr>(BE->getLeft()))
                return Builder->Sub(LHS->Add(CE), BE->getRight());
            // C_0 + (X - C_1) ==> (C_0 - C1) + X
            if (ConstantExpr *CE = dyn_cast<ConstantExpr>(BE->getRight()))
                return Builder->Add(LHS->Sub(CE), BE->getLeft());
            break;
        }
        }
    ...
    ref<Expr> Add(const ref<NonConstantExpr> &LHS,
        ...
        case Expr::Add: {
            ...
            // (X + Y) + Z ==> X + (Y + Z)
            return Builder->Add(BE->getLeft(),
                                Builder->Add(BE->getRight(), RHS));
        }
        ...
        case Expr::Add: {
            ...
            // X + (C_0 + Y) ==> C_0 + (X + Y)
            if (ConstantExpr *CE = dyn_cast<ConstantExpr>(BE->getLeft()))
                return Builder->Add(CE, Builder->Add(LHS, BE->getRight()));
            // X + (Y + C_0) ==> C_0 + (X + Y)
            if (ConstantExpr *CE = dyn_cast<ConstantExpr>(BE->getRight()))
                return Builder->Add(CE, Builder->Add(LHS, BE->getLeft()));
            break;
        }
        ...
        case Expr::Sub: {
            ...
            // X + (C_0 - Y) ==> C_0 + (X - Y)
            if (ConstantExpr *CE = dyn_cast<ConstantExpr>(BE->getLeft()))
                return Builder->Add(CE, Builder->Sub(LHS, BE->getRight()));
            // X + (Y - C_0) ==> -C_0 + (X + Y)
            if (ConstantExpr *CE = dyn_cast<ConstantExpr>(BE->getRight()))
                return Builder->Add(CE->Neg(), Builder->Add(LHS, BE->getLeft()));
            break;
        }
        }
    ...
    ref<Expr> Sub(const ref<ConstantExpr> &LHS,
        ...
        case Expr::Add: {
            ...
            // C_0 - (C_1 + X) ==> (C_0 - C1) - X
            if (ConstantExpr *CE = dyn_cast<ConstantExpr>(BE->getLeft()))
                return Builder->Sub(LHS->Sub(CE), BE->getRight());
            // C_0 - (X + C_1) ==> (C_0 + C1) + X
            if (ConstantExpr *CE = dyn_cast<ConstantExpr>(BE->getRight()))
                return Builder->Sub(LHS->Sub(CE), BE->getLeft());
            break;
        }
        ...
        case Expr::Sub: {
            ...
            // C_0 - (C_1 - X) ==> (C_0 - C1) + X
            if (ConstantExpr *CE = dyn_cast<ConstantExpr>(BE->getLeft()))
                return Builder->Add(LHS->Sub(CE), BE->getRight());
            // C_0 - (X - C_1) ==> (C_0 + C1) - X
            if (ConstantExpr *CE = dyn_cast<ConstantExpr>(BE->getRight()))
                return Builder->Sub(LHS->Add(CE), BE->getLeft());
            break;
        }
        }
    ...
    ref<Expr> Sub(const ref<NonConstantExpr> &LHS,
        ...
        case Expr::Add: {
            BinaryExpr *BE = cast<BinaryExpr>(LHS);
            // (X + Y) - Z ==> X + (Y - Z)
            return Builder->Add(BE->getLeft(), Builder->Sub(BE->getRight(), RHS));
        }

        case Expr::Sub: {
            BinaryExpr *BE = cast<BinaryExpr>(LHS);
            // (X - Y) - Z ==> X - (Y + Z)
            return Builder->Sub(BE->getLeft(), Builder->Add(BE->getRight(), RHS));
        }
        }
        ...
        case Expr::Add: {
            ...
            // X - (C + Y) ==> -C + (X - Y)
            if (ConstantExpr *CE = dyn_cast<ConstantExpr>(BE->getLeft()))
                return Builder->Add(CE->Neg(), Builder->Sub(LHS, BE->getRight()));
            // X - (Y + C) ==> -C + (X + Y)
            if (ConstantExpr *CE = dyn_cast<ConstantExpr>(BE->getRight()))
                return Builder->Add(CE->Neg(), Builder->Sub(LHS, BE->getLeft()));
            break;
        }
        ...
        case Expr::Sub: {
            ...
            // X - (C - Y) ==> -C + (X + Y)
            if (ConstantExpr *CE = dyn_cast<ConstantExpr>(BE->getLeft()))
                return Builder->Add(CE->Neg(), Builder->Add(LHS, BE->getRight()));
            // X - (Y - C) ==> C + (X - Y)
            if (ConstantExpr *CE = dyn_cast<ConstantExpr>(BE->getRight()))
                return Builder->Add(CE, Builder->Sub(LHS, BE->getLeft()));
            break;
        }
        }
...
public:
    ...
    ref<Expr> Not(const ref<NonConstantExpr> &LHS) {
        ...
        if (const OrExpr *OE = dyn_cast<OrExpr>(LHS))
            return Builder->And(Builder->Not(OE->getLeft()),
                                Builder->Not(OE->getRight()));
        return Base->Not(LHS);
    }
