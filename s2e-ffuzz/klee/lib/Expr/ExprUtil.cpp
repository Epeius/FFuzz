...
void klee::findReads(ref<Expr> e,
    ...
    while (!stack.empty()) {
        ...
        if (ReadExpr *re = dyn_cast<ReadExpr>(top)) {
            ...
            if (!isa<ConstantExpr>(re->getIndex()) &&
                    visited.insert(re->getIndex()).second)
            ...
            if (visitUpdates) {
                ...
                if (updates.insert(re->getUpdates().getHead()).second) {
                    for (const UpdateNode *un=re->getUpdates().getHead(); un; un=un->getNext()) {
                        if (!isa<ConstantExpr>(un->getIndex()) &&
                                visited.insert(un->getIndex()).second)
                            stack.push_back(un->getIndex());
                        if (!isa<ConstantExpr>(un->getValue()) &&
                                visited.insert(un->getValue()).second)
