...
void ExprSMTLIBPrinter::printReadExpr(const ref<ReadExpr>& e)
{
    *p << "(" << getSMTLIBKeyword(e) << " ";
    ...
    //print index
    printSeperator();
    printExpression(e->getIndex(),SORT_BITVECTOR);
...
{
    unsigned int lowIndex= e->getOffset();
    unsigned int highIndex= lowIndex + e->getWidth() -1;
...
void ExprSMTLIBPrinter::printCastExpr(const ref<CastExpr>& e)
{
    /* sign_extend and zero_extend behave slightly unusually in SMTLIBv2
                 ...
                 */
    unsigned int numExtraBits= (e->getWidth()) - (e->getSrc()->getWidth());
    ...
    *p << "((_ " << getSMTLIBKeyword(e) << " " <<
    ...
    //recurse
    printExpression(e->getSrc(),SORT_BITVECTOR);
...
void ExprSMTLIBPrinter::printUpdatesAndArray(const UpdateNode* un, const Array* root)
{
    if(un!=NULL)
    {
        *p << "(store ";
        ...
        //print index
        printExpression(un->getIndex(),SORT_BITVECTOR);
        printSeperator();

        //print value that is assigned to this index of the array
        printExpression(un->getValue(),SORT_BITVECTOR);
        ...
        for(set<const Array*>::iterator it = usedArrays.begin(); it != usedArrays.end(); it++)
        {
            ...
            if(array->isConstantArray())
            {
                /*loop over elements in the array and generate an assert statement
                                          for each one
                                         */
                for(vector< ref<ConstantExpr> >::const_iterator ce= array->getConstantValues().begin();
                    ce != array->getConstantValues().end(); ce++, byteIndex++)
                {
                    *p << "(assert (";
                    p->pushIndent();
                    *p <<           "= ";
                    ...
                    *p << "(select " << array->getName() << " (_ bv" << byteIndex << " " << array->getDomain() << ") )";
                    printSeperator();
                    printConstant((*ce));
...
void ExprSMTLIBPrinter::printPush()
{
    *o << "(push)" << endl;
}

void ExprSMTLIBPrinter::printPop()
{
    *o << "(pop)" << endl;
}
...
void ExprSMTLIBPrinter::printReset()
{
    *o << "(reset)" << endl;
}
