...
ref<Expr> ConstantExpr::fromMemory(void *address, Width width) {
    ...
    case Expr::Int128: return ConstantExpr::create(*((__uint128_t*) address), width);
...
void ConstantExpr::toMemory(void *address) {
    ...
    case Expr::Int128: *((__uint128_t*) address) = getZExtValue(128); break;
        // FIXME: Should support long double, at least.
    }
}
...
#define __LIFT_CONST_SELECT_1(e1, ...)                                        \
    ...
    return SelectExpr::create(se->getCondition(),                                     \
    create(se->getTrue(), ## __VA_ARGS__),         \
...
static ref<Expr> TryConstArrayOpt(const ref<ConstantExpr> &cl,
    ...
    for (unsigned i = 0, e = rd->getUpdates().getRoot()->getSize(); i != e; ++i) {
        if (cl == rd->getUpdates().getRoot()->getConstantValues()[i]) {
            ...
            ref<Expr> mayBe =
                    EqExpr::create(rd->getIndex(), ConstantExpr::alloc(i,
                                                                  rd->getIndex()->getWidth()));
    ...
    if (width == Expr::Bool) {
        if (cl->isTrue()) {
            ...
            if (rk == Expr::Eq) {
                ...
                return AndExpr::create(Expr::createIsZero(roe->getLeft()),
                                       Expr::createIsZero(roe->getRight()));
            }
        }
        ...
        if (isa<ConstantExpr>(se->getLeft())) {
            // c0 = c1 - b => c1 - c0 = b
            return EqExpr_createPartialR(cast<ConstantExpr>(SubExpr::create(se->getLeft(),
                                                                            cl)),
                                         se->getRight().get());
        }
