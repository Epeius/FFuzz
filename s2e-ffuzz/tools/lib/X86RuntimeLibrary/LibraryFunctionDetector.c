...
typedef void (*detect_func_t)(revgen_function_t func, uint64_t func_addr,
                              uint8_t *stack,
                              volatile bool *result, FILE *fp);

static detect_func_t s_detectors [] = {
    detect_strlen,
    detect_strcmp,
    detect_strtol,
    detect_output_str,
...
static bool detect_function_type(revgen_function_t func, uint64_t func_addr,
                                 uint8_t *stack, FILE *fp)
{
...
int __detect_library_functions(int argc, char **argv)
{
    ...
    s_stack = (uint8_t *) mmap((void*) NULL, STACK_SIZE, PROT_READ | PROT_WRITE,
