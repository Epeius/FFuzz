...
static int detect_strcmp_invoke(uint8_t *stack, revgen_function_t func, void *b1, unsigned len1,
                         void *b2, unsigned len2, unsigned n)
{
    reset(stack);
                   ...
                   volatile bool *result, FILE *fp)
{
    ...
    for (unsigned i = 0; t[i].str1; ++i) {
        ...
        if (ret < 0) actual = -1;
        else if (ret == 0) actual = 0;
        else actual = 1;

        ret = strcmp(t[i].str1, t[i].str2);
        int expected;
        if (ret < 0) expected = -1;
        else if (ret == 0) expected = 0;
        else expected = 1;

        printf("    actual=%d (0x%x) expected=%d str1='%s' str2='%s'\n", actual, actual_orig, expected, t[i].str1, t[i].str2);
        if (actual != expected) {
            if (actual == -expected) {
                ++swapped_count;
            } else {
...
static long detect_strtol_invoke(uint8_t *stack, revgen_function_t func,
                   ...
                   volatile bool *result, FILE *fp)
{
    ...
    const char *tests[] = {
        "1", "0x12345", "-1234", "12345678912345665432123", NULL
    };
