...
struct IMAGE_IMPORT_DESCRIPTOR {
    ...
    uint32_t TimeDateStamp;             // 0 if not bound or -1 if bound, and
                                        // real date\time stamp in
                                        // IMAGE_DIRECTORY_ENTRY_BOUND_IMPORT
                                        // (new BIND) O.W. date/time stamp of
                                        // DLL bound to (Old BIND)

    uint32_t ForwarderChain;            // -1 if no forwarders
    uint32_t Name;
    uint32_t FirstThunk;                // RVA to IAT (if bound this IAT has
                                        // actual addresses)
} __attribute__((packed));
...
public:
    ...
    const virtual Imports &getImports() const;
    const virtual RelocationEntries &getRelocations() const;
};
