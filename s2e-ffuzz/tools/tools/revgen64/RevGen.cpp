...
namespace {
    cl::opt<std::string>
    BitcodeLibary("bitcodelib",
            cl::desc("Path to the bitcode library"),
            cl::Required);

    cl::opt<std::string>
    BinaryFile("binary",
            cl::desc("The binary file to translate"),
            cl::Required);
    ...
    cl::opt<bool>
    GenTrace("gentrace",
            cl::desc("Generate tracing code for debugging"),
            cl::init(false), cl::Optional);

    cl::opt<bool>
    DetectLibraryFunctions("detect-library-functions",
            cl::desc("Instrument the binary for runtime detection of library functions"),
            cl::init(false), cl::Optional);

    cl::opt<bool>
    EraseTbFunctions("erase-tb-functions",
            cl::desc("Remove needless translation block functions after code generation"),
            cl::init(false), cl::Optional);

    cl::opt<std::string>
    FunctionsToRevgen("functions-to-revgen",
            cl::desc("Only translate the given functions"), cl::Optional);
}
...
{
    m_fp = new vmi::FileSystemFileProvider(m_binaryFile);
    if (!m_fp->open(false)) {
...
void RevGen::InjectArray(Module *m,
                               const std::string &ptrName, const std::string &arrName,
                               SmallVector<Constant*, 4> &arrValues)
{
    ...
    GlobalVariable *arr = new GlobalVariable(*m, arrTy, false,
                             llvm::GlobalVariable::PrivateLinkage,
                             initializer, arrName);
    ...
    if (!var) {
        var = new GlobalVariable(
                        *m, initializer->getType(), false, GlobalValue::ExternalLinkage,
                        initializer, ptrName);
    }
...
void RevGen::generateIndirectJump(llvm::IRBuilder<> &builder,
                                  RevGen::LLVMBasicBlockMap &allBbs,
                                  uint64_t bbPc)
{
    Module *m = m_translator->getModule();
    LLVMContext &ctx = m->getContext();
...
void RevGen::translate(const llvm::BinaryFunctions &functions,
                       const llvm::BinaryBasicBlocks &bbs)
{
        ...
        switch(tb->getType()) {
            ...
            case BB_JMP: {
                ...
                if (basicBlocks.find(tb->getSuccessor(0)) == basicBlocks.end()) {
                    LOGWARNING("Basic block with direct jump " << hexval(tb->getAddress()) << " is missing its successor\n");
            ...
            case BB_COND_JMP: {
                ...
                if (trueBB && falseBB) {
                    ...
                    LOGWARNING("Basic block with conditional branch" << hexval(tb->getAddress()) << " is missing one or more successors\n");
