...
namespace {
    cl::opt<std::string>
    InputBinary("binary",
            cl::desc("The binary file to harden"),
            cl::Required);

    /* For now, only harden single instructions */
    cl::opt<std::string>
    ProgramCounter("pc",
            cl::desc("Instruction to harden"),
            cl::Required);
...
{
    m_fp = new FileSystemFileProvider(m_inputBinaryPath);
    if (!m_fp->open(true)) {
...
uint8_t *Hardening::assemble(const std::string &assembly, unsigned *size)
{
    ...
    binasm = std::unique_ptr<FileSystemFileProvider>(new FileSystemFileProvider(binfile));
    if (!binasm->open(false)) {
...
bool Hardening::harden(uint64_t pc)
{
    ...
    std::stringstream ss;
    ss <<
    "%define LOAD_BASE 0x" << std::hex << m_inputBinary->getImageBase() + freehdr.VirtualAddress << "\n" <<
