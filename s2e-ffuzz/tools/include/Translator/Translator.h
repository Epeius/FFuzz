...
public:
    ...
    TranslatedBlock(uint64_t address, unsigned size,
                    llvm::Function *f, ETranslatedBlockType type,
                    const Successors &succs,
                    const Stores &pcstores) {
        m_address = address;
        m_size = size;
        m_function = f;
        m_type = type;
...
public:
    Translator(const std::string &bitcodeLibrary,
               const vmi::ExecutableFile *binary);
