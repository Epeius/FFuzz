...
void memory_region_init_owner(MemoryRegion *mr,
                        struct Object *owner,
                        const char *name,
                        uint64_t size)
{
...
void memory_region_init_io_owner(MemoryRegion *mr,
                           struct Object *owner,
                           const MemoryRegionOps *ops,
                           void *opaque,
                           const char *name,
                           uint64_t size)
{
    memory_region_init_io(mr, ops, opaque, name, size);
    mr->owner = owner;
}
...
void memory_region_init_ram_ptr_owner(MemoryRegion *mr,
                                struct Object *owner,
                                const char *name,
                                uint64_t size,
                                void *ptr)
{
    memory_region_init_ram_ptr(mr, name, size, ptr);
    mr->owner = owner;
}
...
bool memory_region_is_logging(const MemoryRegion *mr)
{
...
#ifdef CONFIG_SYMBEX
void memory_notify_remap(uintptr_t host_addr,
                         uintptr_t old_va, uintptr_t new_va, unsigned size,
                         bool read_only)
{
    MemoryListener *listener;
    QTAILQ_FOREACH(listener, &memory_listeners, link) {
        if (listener->remap) {
            listener->remap(listener, host_addr, old_va, new_va, size, read_only);
        }
    }
}
#endif
