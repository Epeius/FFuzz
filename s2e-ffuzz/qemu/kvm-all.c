...
static int s_kvm_memory_rw_supported = 0;

static void kvm_init_host_memory_rw(KVMState *s)
{
    int supported = kvm_check_extension(s, KVM_CAP_MEM_RW);
    if (supported > 0) {
        s_kvm_memory_rw_supported = 1;
    }
}

int kvm_host_memory_rw_supported(void)
{
    return kvm_enabled() && s_kvm_memory_rw_supported;
}

int kvm_host_memory_rw(void *dest, const void *source, uint64_t length, bool is_write)
{
    assert(kvm_enabled());
    if (!s_kvm_memory_rw_supported) {
        return -1;
    }

    KVMState *s = kvm_state;
    struct kvm_mem_rw desc;
    desc.is_write = is_write;
    desc.source = (uintptr_t) source;
    desc.dest = (uintptr_t) dest;
    desc.length = length;
    desc.is_dest_guest_phys = false;
    desc.is_source_guest_phys = false;

    if (kvm_vm_ioctl(s, KVM_MEM_RW, &desc) < 0) {
        assert(0 && "KVM KVM_MEM_RW failed");
    }

    return 0;
}
#else
int kvm_host_memory_rw(void *dest, void *source, uint64_t length, bool is_write)
{
    return -1;
}
#endif
