...
static int ioapic_irq_cpu_route(void *opaque, int vector)
{
    ...
    uint8_t ret = entry & IOAPIC_VECTOR_MASK;
    return ret;
}
...
static void ioapic_init(IOAPICCommonState *s, int instance_no)
{
    ...
    qdev_init_gpio_in2(&s->busdev.qdev, ioapic_set_irq, ioapic_irq_cpu_route, IOAPIC_NUM_PINS);
