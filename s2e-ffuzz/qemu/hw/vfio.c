...
#else
#define DPRINTF(fmt, ...) \
    do { s2e_debug_print("vfio: " fmt, ## __VA_ARGS__); } while (0)
#endif
#else
#define DPRINTF(fmt, ...) \
    do { } while (0)
...
#ifdef CONFIG_SYMBEX
#define error_report(...) s2e_debug_print(__VA_ARGS__)
#endif

/* Extra debugging, trap acceleration paths for more logging */
/* S2E: MMAP causes problems when saving/restoring snapshots */
#define VFIO_ALLOW_MMAP 0

/* S2E: we don't use KVM */
#define VFIO_ALLOW_KVM_INTX 0
#define VFIO_ALLOW_KVM_MSI 0
#define VFIO_ALLOW_KVM_MSIX 0

#define kvm_irqchip_release_virq(x, y)
#define kvm_irqchip_add_msi_route(x, y) -1
#define kvm_irqchip_add_irqfd_notifier(a, b, c, d) -1
#define kvm_irqchip_remove_irqfd_notifier(a, b, c)
#define kvm_irqchip_update_msi_route(a, b, c)

#define pci_unregister_vga(a) assert(false && "Not implemented")

#define memory_region_is_iommu(x) false
...
/**
 * The following race condition may happen:
 * - The physical device asserts the interrupt pin
 * - The vfio driver sets the interrupt event
 * - The guest device driver tells the device to mask the interrupt
 * - QEMU fires vfio_intx_interrupt, which asserts the virtual device pin
 * - The guest device driver receives the interrupt.
 *   Since it previously disabled it, it returns without touching the device
 *   (this could legitimately happen if the interrupt is shared)
 * - The virtual device pin is still up, triggering the interrupt again.
 *   => interrupt storm, the guest hangs.
 *
 * The solution:
 * - Deassert the interrupt pin of the virtual device after it is
 *   delivered to the guest.
 *
 * The implementation:
 * - Compute the mapping virtual pci device <=> cpu interrupt vector
 * - The main exec loop will call us, we'll check the vector and
 *   deassert the interrupt pin if the vector is ours.
 */
static void vfio_on_hardware_interrupt(void *opaque, int cpu_vector)
{
    VFIODevice *vdev = opaque;
    if (cpu_vector == vdev->cpu_int_vector) {
        DPRINTF("%s(%04x:%02x:%02x.%x) CPU delivered our interrupt (%#x)\n", __func__, vdev->host.domain,
...
static void vfio_intx_interrupt(void *opaque)
{
    ...
    vdev->cpu_int_vector = pci_irq_get_cpu_vector(&vdev->pdev);
    DPRINTF("  CPU Vector: %d\n", vdev->cpu_int_vector);
...
static void vfio_pci_size_rom(VFIODevice *vdev)
{
    ...
    memory_region_init_io_owner(&vdev->pdev.rom, OBJECT(vdev),
...
static void vfio_vga_probe_ati_3c3_quirk(VFIODevice *vdev)
{
    ...
    memory_region_init_io_owner(&quirk->mem, OBJECT(vdev), &vfio_ati_3c3_quirk, quirk,
...
static void vfio_probe_ati_bar4_window_quirk(VFIODevice *vdev, int nr)
{
    ...
    memory_region_init_io_owner(&quirk->mem, OBJECT(vdev),
...
static void vfio_probe_ati_bar2_4000_quirk(VFIODevice *vdev, int nr)
{
    ...
    memory_region_init_io_owner(&quirk->mem, OBJECT(vdev), &vfio_generic_quirk, quirk,
...
static void vfio_vga_probe_nvidia_3d0_quirk(VFIODevice *vdev)
{
    ...
    memory_region_init_io_owner(&quirk->mem, OBJECT(vdev), &vfio_nvidia_3d0_quirk,
...
static void vfio_probe_nvidia_bar0_88000_quirk(VFIODevice *vdev, int nr)
{
    ...
    memory_region_init_io_owner(&quirk->mem, OBJECT(vdev), &vfio_nvidia_88000_quirk,
...
#ifdef CONFIG_SYMBEX
        assert(false && "Not implemented in S2E mode");
#endif
                                     ...
                                     MemoryRegionSection *section)
{
    ...
    ram_addr_t size = end - iova;
    for (ram_addr_t offset = 0; offset < size; offset += 0x1000) {
        ret = vfio_dma_map(container, iova + offset, 0x1000, vaddr + offset, section->readonly);
        if (ret) {
...
#ifdef CONFIG_SYMBEX
static void vfio_listener_remap(MemoryListener *listener, uintptr_t host_addr, uintptr_t old_va,
                                 uintptr_t new_va, unsigned size, bool read_only)
{
    VFIOContainer *container = container_of(listener, VFIOContainer,
                                            iommu_data.listener);
...
#if 0
    DPRINTF("%s host_addr=%#"PRIxPTR" old_va=%#"PRIxPTR" new_va=%#"PRIxPTR
            " size=%d read_only=%d\n",
            __func__, host_addr, old_va, new_va, size, read_only);
#endif

    if (old_va) {
        ret = vfio_dma_unmap(container, host_addr, size);
        if (ret) {
            error_report("%s unmapping of host_addr=%#"PRIxPTR" old_va=%#"PRIxPTR" new_va=%#"PRIxPTR" size=%#x failed\n",
                         __func__, host_addr, old_va, new_va, size);
        }
    }

    if (new_va) {
        ret = vfio_dma_map(container, host_addr, size, (void*) new_va, read_only);
        if (ret) {
            error_report("%s mapping of host_addr=%#"PRIxPTR" old_va=%#"PRIxPTR" new_va=%#"PRIxPTR" size=%#x failed\n",
                         __func__, host_addr, old_va, new_va, size);
        }
    }
}
...
static MemoryListener vfio_memory_listener = {
    #ifdef CONFIG_SYMBEX
    //Implement some primitive support of copy on write
    .remap = vfio_listener_remap
    #else
                         ...
                         const char *name)
{
    ...
    if (VFIO_ALLOW_MMAP && size && bar->flags & VFIO_REGION_INFO_FLAG_MMAP) {
        ...
        *map = mmap(NULL, size, prot, MAP_SHARED,
        ...
        memory_region_init_ram_ptr_owner(submem, OBJECT(vdev), name, size, *map);
    } else {
empty_region:
        /* Create a zero sized sub-region to make cleanup easy. */
        memory_region_init_owner(submem, OBJECT(vdev), name, 0);
    }
...
static void vfio_map_bar(VFIODevice *vdev, int nr)
{
    ...
    memory_region_init_io_owner(&bar->mem, OBJECT(vdev), &vfio_bar_ops,
                          bar, name, size);
...
static void vfio_map_bars(VFIODevice *vdev)
{
    ...
    if (vdev->has_vga) {
        assert(false && "VGA nor supported");
        memory_region_init_io_owner(&vdev->vga.region[QEMU_PCI_VGA_MEM].mem,
                              OBJECT(vdev), &vfio_vga_ops,
                              &vdev->vga.region[QEMU_PCI_VGA_MEM],
                              "vfio-vga-mmio@0xa0000",
                              QEMU_PCI_VGA_MEM_SIZE);
        memory_region_init_io_owner(&vdev->vga.region[QEMU_PCI_VGA_IO_LO].mem,
                              OBJECT(vdev), &vfio_vga_ops,
                              &vdev->vga.region[QEMU_PCI_VGA_IO_LO],
                              "vfio-vga-io@0x3b0",
                              QEMU_PCI_VGA_IO_LO_SIZE);
        memory_region_init_io_owner(&vdev->vga.region[QEMU_PCI_VGA_IO_HI].mem,
                              OBJECT(vdev), &vfio_vga_ops,
...
static int vfio_connect_container(VFIOGroup *group)
{
    ...
    if (ioctl(fd, VFIO_CHECK_EXTENSION, VFIO_TYPE1_IOMMU)) {
        ...
        memory_listener_register(&container->iommu_data.listener, NULL);
    } else {
...
static int vfio_initfn(PCIDevice *pdev)
{
    ...
    cpu_exec_add_hardware_int_event_handler(&vfio_on_hardware_interrupt, vdev);
...
static int vfio_exitfn(PCIDevice *pdev)
{
    VFIODevice *vdev = DO_UPCAST(VFIODevice, pdev, pdev);
...
void vfio_s2e_switch_to_symbolic(PCIDevice *pdev)
{
    ...
    s2e_debug_print("vfio: switching to symbolic mode\n");
...
PCIDevice *vfio_s2e_find_device(void)
{
    ...
    if (count > 1) {
        error_report("vfio: s2e does not support more than 1 device for now\n");
        exit(-1);
    }
...
static const VMStateDescription vfio_pci_vmstate = {
    .name = "vfio-pci",
    .unmigratable = 0, //S2E hack to allow snapshotting
    .version_id = 2,
    .minimum_version_id = 1,
    .minimum_version_id_old = 1,

    .fields      = (VMStateField []) {
        VMSTATE_PCI_DEVICE(pdev, VFIODevice),
        VMSTATE_UINT32(cpu_int_vector, VFIODevice),
        VMSTATE_UINT32(config_size, VFIODevice),
        VMSTATE_END_OF_LIST()
    }
};
