...
typedef int (*pci_get_irq_cpu_vector_fn)(void *opaque, int irq_num);
...
bool pci_intx_route_changed(PCIINTxRoute *old, PCIINTxRoute *new_);
...
static inline void pci_irq_assert(PCIDevice *pci_dev)
{
    ...
    qemu_set_irq(pci_dev->irq[pin], 1);
}

static inline void pci_irq_deassert(PCIDevice *pci_dev)
{
    int pin = pci_get_byte(pci_dev->config + PCI_INTERRUPT_PIN) - 1;
    qemu_set_irq(pci_dev->irq[pin], 0);
}


static inline int pci_irq_get_cpu_vector(PCIDevice *pci_dev)
{
    int pin = pci_get_byte(pci_dev->config + PCI_INTERRUPT_PIN) - 1;
    return qemu_get_irq_cpu_vector(pci_dev->irq[pin]);
}
