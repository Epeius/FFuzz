...
void qdev_init_gpio_in2(DeviceState *dev, qemu_irq_handler handler,
                        qemu_irq_route route, int n)
{
    assert(dev->num_gpio_in == 0);
    dev->num_gpio_in = n;
    dev->gpio_in = qemu_allocate_irqs2(handler, route, dev, n);
}

void qdev_init_gpio_in(DeviceState *dev, qemu_irq_handler handler, int n)
{
    qdev_init_gpio_in2(dev, handler, NULL, n);
}
