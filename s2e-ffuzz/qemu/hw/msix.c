              ...
              uint8_t pba_bar_nr, unsigned pba_offset, uint8_t cap_pos)
{
    ...
    memory_region_init_io_owner(&dev->msix_table_mmio, OBJECT(dev), &msix_table_mmio_ops, dev,
                          "msix-table", table_size);
    memory_region_add_subregion(table_bar, table_offset, &dev->msix_table_mmio);
    memory_region_init_io_owner(&dev->msix_pba_mmio, OBJECT(dev), &msix_pba_mmio_ops, dev,
                          "msix-pba", pba_size);
...
#define MSIX_EXCLUSIVE_CAP_OFFSET 0
    ...
    memory_region_init_owner(&dev->msix_exclusive_bar, OBJECT(dev), name, MSIX_EXCLUSIVE_BAR_SIZE);
    g_free(name);
