...
static void code_gen_init_precise_excep(void)
{
    ...
    if (code_gen_precise_excp_count == 0) {
        ...
        //s2e_debug_print("Resizing precise exception buffer from %d to %d entries\n", code_gen_precise_excp_count, code_gen_precise_excp_count * 2);
        code_gen_precise_excp_count *= 2;
        code_gen_precise_excp_max_count = code_gen_precise_excp_count - 0x1000;
        code_gen_precise_excp_buffer = g_realloc(code_gen_precise_excp_buffer, code_gen_precise_excp_count * sizeof(tb_precise_pc_t));
        code_gen_precise_excp_ptr = code_gen_precise_excp_buffer;
    }
}
...
static TranslationBlock *tb_alloc(target_ulong pc)
{
    ...
    if (nb_tbs >= code_gen_max_blocks ||
        ...
        code_gen_precise_excep_flush_needed() ||
        cpu_gen_flush_needed())
#else
    if (nb_tbs >= code_gen_max_blocks ||
        (code_gen_ptr - code_gen_buffer) >= code_gen_buffer_max_size)
#endif
...
void s2e_fix_code_gen_ptr(TranslationBlock* tb, int code_gen_size)
{
    code_gen_ptr = (void *)(((unsigned long)code_gen_ptr + code_gen_size + CODE_GEN_ALIGN - 1) & ~(CODE_GEN_ALIGN - 1));
    code_gen_precise_excp_ptr += tb->icount;
    ++code_gen_precise_excp_ptr;
...
#ifdef CONFIG_SYMBEX
            target_ulong iovaddr = s2e_read_mem_io_vaddr(1);
    ...
    for (mmu_idx = 0; mmu_idx < NB_MMU_MODES; mmu_idx++) {
        ...
        if (addr == (tlb_entry->addr_read &
            ...
            addr == (tlb_entry->addr_code &
                     (TARGET_PAGE_MASK | TLB_INVALID_MASK))) {
#if defined(CONFIG_SYMBEX)
            se_flush_tlb_cache_page(env->tlb_table[mmu_idx][i].objectState, mmu_idx, i);
#endif
            *tlb_entry = s_cputlb_empty_entry;
        }
    }
...
static inline int tlb_is_dirty(CPUArchState *env, target_ulong vaddr)
{
    int i;
    int mmu_idx;
    int ret = 1;
                  ...
                  int mmu_idx, target_ulong size)
{
...
#ifdef CONFIG_SYMBEX
    if (se_is_mmio_symbolic(section->mr, paddr, 1LL << TARGET_PAGE_BITS)) {
    ...
    if (addend) {
        ...
        s2e_update_tlb_entry(env, mmu_idx, vaddr, addend);
    }
#endif
...
uintptr_t s2e_notdirty_mem_read(target_phys_addr_t ram_addr)
{
    uintptr_t ret = (uintptr_t)qemu_get_ram_ptr(ram_addr);
    //s2e_debug_print("s2e_notdirty_mem_read: %llx => %llx\n", ram_addr, ret);
    return ret;
}
...
static uint64_t s2edma_mem_read(void *opaque, target_phys_addr_t ram_addr, unsigned size)
{
    switch(size) {
        ...
        case 8: return ldq_raw(qemu_get_ram_ptr(ram_addr));
    }
    assert(false && "Invalid size");
    return 0;
}

static void s2edma_mem_write(void *opaque, target_phys_addr_t ram_addr,
                               uint64_t val, unsigned size)
{
    target_ulong iovaddr = s2e_read_mem_io_vaddr(1);
    if (!tlb_is_dirty(cpu_single_env, iovaddr)) {
        notdirty_mem_write(opaque, ram_addr, val, size);
        return;
    }
...
{
    if (isWrite) {
        return mr->ops->write == s2edma_mem_write;
    } else {
...
#ifdef CONFIG_SYMBEX
    vaddr = (s2e_read_mem_io_vaddr(1)) + offset;
#else
...
static void io_mem_init(void)
{
    ...
    #ifdef CONFIG_SYMBEX
    memory_region_init_io(&io_mem_notdirty, &s2edma_mem_ops, NULL,
                          "notdirty", UINT64_MAX);
    #else
    ...
    while (len > 0) {
        ...
        if (is_write) {
...
#else
                if (!kvm_enabled() ||  kvm_host_memory_rw(ptr, buf, l, 1) < 0) {
                    memcpy(ptr, buf, l);
                }
#endif
            ...
            if (!is_ram_rom_romd(section)) {
                ...
                /* RAM case */
                ram_addr_t addr1 = section->mr->ram_addr
                        + section_addr(section, addr);
                ptr = qemu_get_ram_ptr(addr1);
#ifdef CONFIG_SYMBEX
                s2e_dma_read((uintptr_t)ptr, buf, l);
#else
                if (!kvm_enabled() ||  kvm_host_memory_rw(buf, ptr, l, 0) < 0) {
                    memcpy(buf, ptr, l);
                }
#endif
                qemu_put_ram_ptr(ptr);
    ...
    while (len > 0) {
        ...
        if (!is_ram_rom_romd(section)) {
            ...
            for(i=0; i<l; ++i)
                stb_raw(ptr+i, buf[i]);
#else
            /* KVM library must intercept all read/write ops */
            if (!kvm_enabled() || kvm_host_memory_rw(ptr, buf, l, true) < 0) {
                memcpy(ptr, buf, l);
            }
#endif
            qemu_put_ram_ptr(ptr);
...
void *cpu_physical_memory_map(target_phys_addr_t addr,
                              ...
                              int is_write)
{
    bool use_bounce_buffers = kvm_host_memory_rw_supported();
#ifdef CONFIG_SYMBEX
