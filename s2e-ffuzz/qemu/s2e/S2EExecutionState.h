...
protected:
    ...
    /* Used to filter redundant testcases */
    ...
    */
    ...
    void addressSpaceChangeUpdateRegisters(const klee::MemoryObject *mo,
                                     const klee::ObjectState *oldState,
                                     klee::ObjectState *newState);
...
public:
    virtual void addressSpaceSymbolicStatusChange(klee::ObjectState *object,
                                         bool becameConcrete);
...
public:
    ...
    uint64_t getInputSize(void) const { return m_InputSize; }

    void setInputSize(uint64_t size) { m_InputSize = size; }
    ...
    inline void zombify() { m_zombie = true; }
    ...
    inline bool isRunningConcrete() const { return m_runningConcrete; }
    ...
    bool testConstraints(const std::vector<klee::ref<klee::Expr> > &c, klee::ConstraintManager *newConstraints = NULL, klee::Assignment *newConcolics = NULL);
    ...
    std::vector<klee::ref<klee::Expr> > createSymbolicArray(
                const std::string& name = std::string(), unsigned size = 4, std::string *varName = NULL);
    ...
    std::vector<klee::ref<klee::Expr> > createConcolicArray(
                ...
                const std::vector<unsigned char> &concreteBuffer, std::string *varName = NULL);
    ...
    template <typename T>
    bool readPointer(uint64_t address, T &value) {
        bool status = false;
        if (getPointerSize() == 4) {
            uint32_t pointer = 0;
