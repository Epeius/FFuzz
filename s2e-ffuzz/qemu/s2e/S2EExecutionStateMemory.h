...
protected:
    ...
    void transferRamInternalSymbolic(klee::ObjectPair op,
                                     uint64_t object_offset, klee::ref<klee::Expr> *buf,
                                     uint64_t size, bool write);

    void transferRamInternal(klee::ObjectPair op,
                             uint64_t object_offset, uint8_t *buf, uint64_t size,
...
public:
    ...
    /** Read/write from physical memory, concretizing if necessary on reads.
        ...
        by qemu_get_ram_ptr. Used by softmmu code. */
    void transferRam(struct CPUTLBRAMEntry *te,
                     uint64_t hostAddress, void *buf, uint64_t size, bool isWrite,
    ...
    bool writeMemoryConcrete(uint64_t address, const void *buf,
    ...
    template <typename T>
    bool writeMemory(uint64_t address, T value,
                     AddressType addressType = VirtualAddress) {
