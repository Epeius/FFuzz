...
public:
    ...
    void registerSharedExternalObject(S2EExecutionState *state,
    ...
    static void doInterruptAll(int intno, int is_int, int error_code,
    ...
    StatePair forkCondition(S2EExecutionState *state, klee::ref<klee::Expr> condition,
            bool keepConditionTrueInCurrentState = false, bool addCondition = true);
    ...
    klee::ref<klee::ConstantExpr> simplifyAndGetExample(S2EExecutionState *state,
...
protected:
    ...
    static void handlerWriteMemIoVaddr(klee::Executor* executor,
                                    klee::ExecutionState* state,
                                    klee::KInstruction* target,
                                    std::vector<klee::ref<klee::Expr> > &args);

    static void handlerBeforeMemoryAccess(klee::Executor* executor,
                                    klee::ExecutionState* state,
                                    klee::KInstruction* target,
                                    std::vector<klee::ref<klee::Expr> > &args);

    static void handlerAfterMemoryAccess(klee::Executor* executor,
                                    klee::ExecutionState* state,
                                    klee::KInstruction* target,
                                    std::vector<klee::ref<klee::Expr> > &args);
