...
#ifdef CONFIG_LIBS2E
//TODO: fix this in vanilla qemu too
        ...
        uint64_t insPc, int staticTarget, uint64_t targetPc)
{
    ...
    try {
        g_s2e->getCorePlugin()->onTranslateBlockEnd.emit(
                signal, g_s2e_state, tb, insPc,
...
void s2e_on_translate_block_complete(TranslationBlock *tb, uint64_t pc)
{
    ...
    try {
        g_s2e->getCorePlugin()->onTranslateBlockComplete.emit(
                g_s2e_state, tb, pc);
    } catch(s2e::CpuExitException&) {
        fast_longjmp(env->jmp_env, 1);
    }
        ...
        TranslationBlock *tb, uint64_t pc, int jump_type)
{
    ...
    try {
        g_s2e->getCorePlugin()->onTranslateJumpStart.emit(signal, g_s2e_state, tb,
                                                        pc, jump_type);
        if(!signal->empty()) {
            s2e_gen_pc_update(context, pc, tb->cs_base);
            s2e_tcg_instrument_code(signal, pc - tb->cs_base);
...
void s2e_on_translate_lea_rip_relative(
        void *context, TranslationBlock *tb, uint64_t pc, uint64_t addr) {
    assert(g_s2e_state->isActive());
    ...
    assert(signal->empty());
    try {
        g_s2e->getCorePlugin()->onTranslateLeaRipRelative.emit(signal,
                                                               g_s2e_state, tb,
                                                               pc, addr);
...
static void s2e_timer_cb(void *opaque)
{
    ...
    c->onTimer.emit();
    qemu_mod_timer(s_timer, qemu_get_clock_ms(rt_clock) + 1000);
}
...
void s2e_init_timers()
{
    ...
    assert(cp);
    s_timer = qemu_new_timer_ms(rt_clock, s2e_timer_cb, cp);
    qemu_mod_timer(s_timer, qemu_get_clock_ms(rt_clock) + 1000);
}
        ...
        unsigned flags, uintptr_t retaddr)
{
    ...
    try {
        g_s2e->getCorePlugin()->onConcreteDataMemoryAccess.emit(g_s2e_state,
            vaddr, value, size, flags);
    } catch(s2e::CpuExitException&) {
        fast_longjmp(env->jmp_env, 1);
    }
}
...
{
    g_s2e->getCorePlugin()->onDeviceRegistration.emit();
}

void s2e_on_device_activation(int bus_type, void *bus)
{
    g_s2e->getCorePlugin()->onDeviceActivation.emit(bus_type, bus);
}
        ...
        int isWrite, void *retaddr)
{
    ...
    try {
        g_s2e->getCorePlugin()->onPortAccess.emit(g_s2e_state,
