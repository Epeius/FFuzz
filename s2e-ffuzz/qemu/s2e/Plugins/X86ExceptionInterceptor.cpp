    ...
    case TASK_GATE:
        {
            ...
            getDebugStream() << "Registered task hander at " <<
                    hexval(eip) << '\n';
    ...
    case INT_GATE:
        {
            ...
            getDebugStream() << "Registered interrupt gate hander at " <<
                    hexval(hdlr.pc) << '\n';
        }
        break;
    ...
    case TRAP_GATE:
        {
            ...
            getDebugStream() << "Registered trap gate hander at " <<
                    hexval(hdlr.pc) << '\n';
        }
        break;

    default:
        getDebugStream() << "Unhandled entry type " << entry.getType() << '\n';
        return false;
        break;
    }
    return true;
...
{
    getInfoStream() << "Fired exception at " << hexval(pc) << '\n';
    ...
    switch(h.idtVector) {
        case GPF:
            getInfoStream() << "General protection fault\n";
            break;
        case STACK_FAULT:
            getInfoStream() << "Stack fault\n";
            break;
        case DOUBLE_FAULT:
            getInfoStream() << "Double fault\n";
            break;
    }
    ...
    switch (entry.getType()) {
        ...
        case INT_GATE:
            getDebugStream() << "INT_GATE not handled yet\n";
            break;

        case TRAP_GATE:
            getDebugStream() << "TRAP_GATE not handled yet\n";
            break;
    }
#if 0
    switch(h.idtVector) {
...
void X86ExceptionInterceptor::handleTaskGate(S2EExecutionState *state, const Handler &hdlr)
{
    ...
    if (!X86Parser::getCurrentTss(state, &tss)) {
        getDebugStream() << "Could not read current TSS\n";
        return;
    }
    ...
    if (!X86Parser::getGdtEntry(state, &gdtEntry, faultyTssSel)) {
        getDebugStream() << "Could not retrieve TSS descriptor for faulty task " << faultyTssSel << '\n';
        return;
    }

    uint32_t faultyTssBase = X86Parser::getBase(gdtEntry);
    if (!X86Parser::getTss(state, faultyTssBase, &faultyTss)) {
        getDebugStream() <<  "Could not get faulty TSS\n";
        return;
    }

    faultyTss.dumpInfo(getDebugStream());

    ModuleExecutionDetector *m_exec = (ModuleExecutionDetector*)s2e()->getPlugin("ModuleExecutionDetector");

    if (m_exec) {
        m_exec->dumpMemory(state, getDebugStream(), faultyTss.m_ESP, 512);
    }else {
