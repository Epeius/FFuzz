...
static const int LINUX_SIG_CHILD = 17;
...
enum BugType {
    SEGFAULT, ILLEGAL_INSTRUCTION, ABORT, STACK_OVERFLOW,
    ARITHMETIC_ERROR, KERNEL_CRASH, USER_MODE_CRASH,
    USER_TRIGGERED_CRASH, RESOURCE_LEAK,
    OTHER, NONFATAL
};

enum BugCollectorType {
    BUG_TYPE_BSOD, BUG_TYPE_CUSTOM, BUG_TYPE_WIN_UM_CRASH,
    BUG_TYPE_RESOURCE_LEAK
};
...
struct BugCollectorBug {
    ...
    /* This bug was updated since the last time it was
       sent to s2e-lb */
...
private:
    ...
    WindowsMonitor2 *m_windowsMonitor;
    ...
    void onMonitorCommand(Monitor *mon, const QDict *command, QDict *ret);

    std::string compressFile(const std::string &path);

    void onTranslateSoftInterruptStart(ExecutionSignal *signal, S2EExecutionState *state,
    ...
    /* Custom opcodes                                */
    /*************************************************/
    void opcodeCustomBug(S2EExecutionState *state,
                         uint64_t guestDataPtr,
                         const S2E_BUG_COMMAND &command);

    void opcodeWindowsUserModeBug(S2EExecutionState *state,
                                  uint64_t guestDataPtr,
                                  const S2E_BUG_COMMAND &command);
