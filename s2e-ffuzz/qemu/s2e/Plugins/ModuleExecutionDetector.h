...
enum S2E_MODEX_DETECTOR_COMMANDS {
    GET_MODULE
};
...
private:
    ...
    void onTranslateBlockComplete(S2EExecutionState *state,
        TranslationBlock *tb,
        uint64_t pc);
    ...
    void handleOpcodeGetModule(S2EExecutionState *state,
...
public:
    ...
    const ModuleDescriptor *getModule(S2EExecutionState *state, uint64_t addressSpace, uint64_t pc, bool tracked=true);
    ...
    std::vector<const ModuleDescriptor*> getModules(S2EExecutionState *state, uint64_t addressSpace, bool tracked=true);
