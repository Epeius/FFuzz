...
void ControlFlowGraph::loadConfiguration()
{
    ...
    foreach2(it, keys.begin(), keys.end()) {
        ...
        if (!ok) {
            getWarningsStream() << ss.str() << ".functions" << " is not a valid list\n";
            exit(-1);
        }
...
void ControlFlowGraph::handleOpcodeInvocation(S2EExecutionState *state,
                                    ...
                                    uint64_t guestDataSize)
{
    ...
    if (guestDataSize != sizeof(command)) {
        getWarningsStream(state) <<
                "mismatched S2E_CFG_COMMAND size\n";
        return;
    }
    ...
    switch (command.Command) {
        case CFG_REGISTER_FUNCTION: {
            ...
            if (!desc) {
                getWarningsStream(state) <<
                    "could not resolve module for address " <<
            ...
            if (!state->mem()->readString(command.Function.FunctionName, FunctionName)) {
                getWarningsStream(state) <<
                    "could not read name for function " <<
                    hexval(command.Function.RunTimeFunctionAddress) << "\n";
                break;
            }
            ...
            getInfoStream(state) <<
                "Registered function " <<
                desc->Name << ":" << hexval(RelFunctionPc) << "(" << FunctionName << ")\n";
        } break;
