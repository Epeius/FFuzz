...
S2E_DEFINE_PLUGIN(StackMonitor, "Tracks stack usage by modules", "StackMonitor",
                  "Interceptor", "ProcessExecutionDetector");
...
public:
    ...
    public:
        Stack(StackMonitorState *plgState, S2EExecutionState *state,
              uint64_t stackBound,
        ...
        void newFrame(StackMonitorState *plgState, S2EExecutionState *state,
                      uint64_t pc, uint64_t stackPointer, unsigned initialFrameSize) {
            if (m_frames.size() > 0) {
                const StackFrame &last = m_frames.back();
                s2e_assert(state, stackPointer < last.top + last.size,
                        "New frame " << hexval(stackPointer) << "is not below the last one " << hexval(last.top + last.size));
            }
        ...
        void update(StackMonitorState *plgState, S2EExecutionState *state, uint64_t pc, uint64_t stackPointer) {
            ...
            while (stackPointer > last->top) {
                ...
                last = &m_frames.back();
                plgState->m_stackMonitor->onStackFrameDelete.emit(state, oldBottom, oldTop, last->top - last->size, last->top);
            }
            ...
            if (oldSize != newSize) {
                ...
                if (newSize > oldSize)
                    plgState->m_stackMonitor->onStackFrameGrow.emit(state, last->top - oldSize, last->top - newSize, last->top);
                else
                    plgState->m_stackMonitor->onStackFrameShrink.emit(state, last->top - oldSize, last->top - newSize, last->top);
            }
        }
...
{
    os << "  Frame pc=" << hexval(frame.pc)
            << " top=" << hexval(frame.top)
            << " size=" << hexval(frame.size);
    return os;
}
...
void StackMonitor::onTranslateBlockStart(ExecutionSignal *signal,
...
void StackMonitor::onTranslateBlockComplete(
        S2EExecutionState* state,
        TranslationBlock *tb,
        uint64_t endPc)
{
...
void StackMonitorState::onProcessUnload(S2EExecutionState *state, uint64_t pid)
{
    ...
    foreach2(it, m_stacks.begin(), m_stacks.end())
    {
        ...
        s2e_assert(state, p.first != pid, "Stack was not deleted for pid " << hexval(p.first) << " tid " << hexval(p.second));
    }
}
...
bool StackMonitorState::getCallStack(S2EExecutionState *state, uint64_t pid, uint64_t tid, StackMonitor::CallStack &callStack) const
{
