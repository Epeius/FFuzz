...
S2E_DEFINE_PLUGIN(BugCollector, "This plugin centralizes all the bugs found by various plugins and makes them available in a standard format", "");


void BugCollector::initialize()
{
    m_linuxMonitor = static_cast<LinuxMonitor*>(s2e()->getPlugin("LinuxMonitor"));
    m_windowsMonitor = static_cast<WindowsMonitor2*>(s2e()->getPlugin("WindowsMonitor2"));
    ...
    if (m_bsodInterceptor) {
        m_bsodInterceptor->onBlueScreen.connect(
                sigc::mem_fun(*this, &BugCollector::onBlueScreen));
    }

    m_compressDumps = s2e()->getConfig()->getBool(getConfigKey() + ".compressDumps", true);

    s2e()->getCorePlugin()->onMonitorEvent.connect(
            sigc::mem_fun(*this, &BugCollector::onMonitorEvent));

    s2e()->getCorePlugin()->onMonitorCommand.connect(
            sigc::mem_fun(*this, &BugCollector::onMonitorCommand));

    s2e()->getCorePlugin()->onTimer.connect(
            sigc::mem_fun(*this, &BugCollector::onTimer));
...
QObject *BugCollectorBug::serialize() const
{
    ...
    foreach2(it, concreteInputs.begin(), concreteInputs.end()) {
        ...
        foreach2(lit, (*it).second.begin(), (*it).second.end()) {
            qlist_append(data, qint_from_int(*lit));
        }
...
bool BugCollector::isNewBug(BugCollectorBug *bug)
{
    ...
    getDebugStream() << "BugCollector: "
            << (b ? "new bug" : "repeated bug") << "\n";
    return b;
}
...
void BugCollector::onMonitorCommand(Monitor *mon, const QDict *command, QDict *ret)
{
    QDict *data = QEMUEvents::getPluginData(this, command);
    if (!data) {
        return;
    }

    if (!qdict_haskey(data, "bugs")) {
        getWarningsStream() << "BugCollector: invalid command "
                << "no bsod key present\n";
        return;
    }

    getDebugStream() << "BugCollector: processing new bugs\n";
    QList *bugs = qobject_to_qlist(qdict_get(data, "bugs"));

    QListEntry *le;
    QLIST_FOREACH_ENTRY(bugs, le) {
        QDict *val = qobject_to_qdict(qlist_entry_obj(le));
        BugCollectorBug *bug = BugCollectorBug::deserialize(val);
        if (!bug) {
            continue;
        }
...
void BugCollector::onMonitorEvent(const QDict *event, QDict *ret)
{
    ...
    QObject *bugs = getUpdatedBugs();
    if (bugs) {
        QDict *result = QEMUEvents::createResult(this, ret);
        qdict_put_obj(result, "bugs", bugs);
    }
}

/*****************************************************************/
void BugCollector::opcodeCustomBug(S2EExecutionState *state,
                                  ...
                                  const S2E_BUG_COMMAND &command)
{
    ...
    if (!ret) {
        getWarningsStream(state) <<
                "BugCollector: could not read custom bug description\n";
        return;
    }
...
void BugCollector::opcodeWindowsUserModeBug(S2EExecutionState *state,
                                  uint64_t guestDataPtr,
                                  const S2E_BUG_COMMAND &command)
{
    BugWindowsUserModeCrash *bug = new BugWindowsUserModeCrash();
    ...
    if (!ret) {
        getWarningsStream(state) <<
                "BugCollector: could not read custom program name\n";
        return;
    }
...
void BugCollector::handleOpcodeInvocation(S2EExecutionState *state,
                            uint64_t guestDataPtr,
                            uint64_t guestDataSize)
{
    ...
    if (guestDataSize != sizeof(command)) {
        getWarningsStream(state) <<
                "BugCollector: mismatched S2E_BUG_COMMAND size\n";
        return;
    }

    if (!state->mem()->readMemoryConcrete(guestDataPtr, &command, guestDataSize)) {
        getWarningsStream(state) <<
                "BugCollector: could not read transmitted data\n";
        return;
    }
