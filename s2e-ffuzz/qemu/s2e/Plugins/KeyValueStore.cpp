...
void KeyValueStore::initialize()
{
    ...
    s2e()->getCorePlugin()->onProcessFork.connect(
        sigc::mem_fun(*this, &KeyValueStore::onProcessFork)
    );
}
...
void KeyValueStore::connectNative()
{
    try {
        ...
        std::cerr << "Exception while connecting to the key value store service: "
                << e.what() << "\n";
        exit(-1);
    }
}
...
void KeyValueStore::connectMemcached()
{
    ...
    if (rc != MEMCACHED_SUCCESS) {
        getWarningsStream() << "KeyValueStore: could not initialize memcached - "
                << memcached_strerror(m_cache, rc) << "\n";
        exit(-1);
    }
...
void KeyValueStore::handleOpcodeInvocation(S2EExecutionState *state,
                                    ...
                                    uint64_t guestDataSize)
{
    ...
    if (guestDataSize != sizeof(command)) {
        getWarningsStream(state)
                << "KeyValueStore: mismatched S2E_KVSTORE_PLUGIN_COMMAND size "
    ...
    if (!state->mem()->readMemoryConcrete(guestDataPtr, &command, guestDataSize)) {
        getWarningsStream(state) <<
                "KeyValueStore: could not read transmitted data\n";
        return;
    }
    ...
    if (command.Command & KVS_INTEGER_KEY) {
        ...
        if (command.KeyAddress && !state->mem()->readString(command.KeyAddress, key)) {
            getWarningsStream(state) <<
                    "KeyValueStore: could not read the key\n";
            return;
        }
    }
    ...
    switch (command.Command & ~KVS_INTEGER_KEY) {
        case KVS_PUT_STRING: {
            if (!state->mem()->readString(command.ValueAddress, value, command.ValueSize)) {
                getWarningsStream(state) <<
                        "KeyValueStore: could not read the value for key " << key << "\n";
                unlock();
                return;
            }
        ...
        default: {
            getWarningsStream(state) <<
                "KeyValueStore: unknown command " << command.Command << "\n";
        } break;
    }
