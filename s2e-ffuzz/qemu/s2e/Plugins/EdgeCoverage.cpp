...
void EdgeCoverage::initialize()
{
    ...
    s2e()->getCorePlugin()->onUpdateStates.connect(
            sigc::mem_fun(*this, &EdgeCoverage::onUpdateStates));

    m_edgeDetector->onEdge.connect(
            sigc::mem_fun(*this, &EdgeCoverage::onEdge));
}


void EdgeCoverage::onUpdateStates(S2EExecutionState* state,
...
void EdgeCoverage::onEdge(S2EExecutionState *state, uint64_t source, EdgeType type)
{
    ...
    if (found) {
        getDebugStream(state) << "Covered edge "
                              << hexval(e.first)
                              << " " << hexval(e.second)
                              << "\n";
...
void EdgeCoverage::addNonCoveredEdge(S2EExecutionState *state, const std::string &module, uint64_t source, uint64_t dest)
{
    ...
    getDebugStream(state) << "Adding non-covered edge "
                          << hexval(info.edge.first)
                          << " " << hexval(info.edge.second)
                          << "\n";
    ...
    foreach2(it, m_nonCoveredEdges.begin(), m_nonCoveredEdges.end()) {
        ...
        foreach2(bit, bypc.begin(), bypc.end()) {
            ...
            if (filter.find(state) != filter.end()) {
                getDebugStream()  << "State id " << (*bit).state->getID()
                        << " has not covered edge "
                        << hexval((*bit).edge.first)
                        << ", " << hexval((*bit).edge.second)
                        << " yet\n";
                return (*bit).state;
            }
        }
