...
void LuaInstructionAnnotation::initialize()
{
    ...
    m_detector->onMonitorLoad.connect(
                sigc::mem_fun(*this, &LuaInstructionAnnotation::onMonitorLoad));
...
bool LuaInstructionAnnotation::registerAnnotation(const std::string &moduleId, const Annotation &annotation)
{
    ...
    getDebugStream() << "LuaInstructionAnnotation: loaded "
                            << moduleId << " "
...
void LuaInstructionAnnotation::onTranslateBlockStart(
        ...
        uint64_t pc)
{
    ...
    m_instructionStart = plg->onTranslateInstructionStart.connect(
        sigc::bind(
            sigc::mem_fun(*this, &LuaInstructionAnnotation::onTranslateInstructionStart),
            (*it).second,
...
void LuaInstructionAnnotation::onTranslateInstructionStart(
        ExecutionSignal *signal,
        S2EExecutionState* state,
        TranslationBlock *tb,
        ...
        uint64_t addend)
{
    ...
    signal->connect(
        sigc::bind(
            sigc::mem_fun(*this, &LuaInstructionAnnotation::onInstruction),
            annotations,
            modulePc
        )
    );
...
void LuaInstructionAnnotation::onTranslateBlockComplete(
        S2EExecutionState *state,
        TranslationBlock *tb,
        uint64_t ending_pc)
{
                                             ...
                                             const ModuleAnnotations *annotations, uint64_t modulePc)
{
    ...
    getDebugStream(state)
            << "LuaInstructionAnnotation: instruction " << hexval(modulePc) << " triggered annotation "
            << (*it).annotationName << "\n";
...
void LuaInstructionAnnotation::handleOpcodeInvocation(S2EExecutionState *state,
                                                   ...
                                                   uint64_t guestDataSize)
{
    ...
    if (guestDataSize != sizeof(command)) {
        getWarningsStream(state) <<
                "LuaInstructionAnnotation: mismatched S2E_LUA_INS_ANN_COMMAND size\n";
        return;
    }

    if (!state->mem()->readMemoryConcrete(guestDataPtr, &command, guestDataSize)) {
        getWarningsStream(state) <<
                "LuaInstructionAnnotation: could not read transmitted data\n";
        return;
    }
    ...
    switch (command.Command) {
        ...
        default: {
            getWarningsStream(state) <<
                    "LuaInstructionAnnotation: incorrect command " << command.Command << "\n";
        }
    }
