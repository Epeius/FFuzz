...
S2E_DEFINE_PLUGIN(MemoryMap, "MemoryMap S2E plugin", "", "WindowsMonitor2");
...
static MemoryMapRegionType WindowsProtectionToInternal(uint64_t protection)
{
    ...
    switch (protection) {
        case PAGE_NOACCESS: type |= MM_NONE; break;
        case PAGE_READONLY: type |= MM_READ; break;
        case PAGE_READWRITE: type |= MM_READ | MM_WRITE; break;
        case PAGE_WRITECOPY: type |= MM_READ | MM_WRITE; break;
        default: assert(false && "Unknown protection flag");
    }
...
public:
    ...
    typedef llvm::DenseMap<uint64_t, WindowsMonitor2::MemoryInformation> MemoryInfoMap;
    ...
    void updateMemoryStats(const WindowsMonitor2::MemoryInformation &stats, uint64_t pid)  {
...
void MemoryMap::initialize()
{
    ...
    m_monitor->onProcessUnload.connect(
            sigc::mem_fun(*this,
                    &MemoryMap::onProcessUnload)
            );

    m_monitor->onNtAllocateVirtualMemory.connect(
            sigc::mem_fun(*this, &MemoryMap::onNtAllocateVirtualMemory));

    m_monitor->onNtFreeVirtualMemory.connect(
            sigc::mem_fun(*this, &MemoryMap::onNtFreeVirtualMemory));

    m_monitor->onNtProtectVirtualMemory.connect(
            sigc::mem_fun(*this, &MemoryMap::onNtProtectVirtualMemory));

    m_monitor->onNtMapViewOfSection.connect(
            sigc::mem_fun(*this, &MemoryMap::onNtMapViewOfSection));

    m_monitor->onNtUnmapViewOfSection.connect(
            sigc::mem_fun(*this, &MemoryMap::onNtUnmapViewOfSection));
}
...
void MemoryMap::onNtAllocateVirtualMemory(S2EExecutionState *state, const S2E_WINMON2_ALLOCATE_VM &d)
{
    ...
    getDebugStream() << __FUNCTION__
                       << " pid=" << hexval(pid)
                       << " target pid=" << hexval(target_pid)
                       << " base=" << hexval(d.BaseAddress)
                       << " size=" << hexval(d.Size)
...
void MemoryMap::addRegion(S2EExecutionState *state, uint64_t pid, uint64_t address, uint64_t end, MemoryMapRegionType type)
{
    DECLARE_PLUGINSTATE(MemoryMapState, state);
    if (plgState->m_trackedPids.find(pid) == plgState->m_trackedPids.end()) {
        return;
...
{
    WindowsMonitor2::MemoryInformation info;
