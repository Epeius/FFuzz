...
int LuaS2EExecutionStateRegisters::read(lua_State *L)
{
    ...
    switch(size) {
        case 1: value = m_state->regs()->read<uint8_t>(pointer); break;
        case 2: value = m_state->regs()->read<uint16_t>(pointer); break;
        case 4: value = m_state->regs()->read<uint32_t>(pointer); break;
        case 8: value = m_state->regs()->read<uint64_t>(pointer); break;
        default: {
            std::stringstream ss;
...
int LuaS2EExecutionStateRegisters::write(lua_State *L)
{
    ...
    if (lua_isuserdata(L, 2)) {
        ...
        switch(size) {
            case 1: m_state->regs()->write<uint8_t>(pointer, value); break;
            case 2: m_state->regs()->write<uint16_t>(pointer, value); break;
            case 4: m_state->regs()->write<uint32_t>(pointer, value); break;
            case 8: m_state->regs()->write<uint64_t>(pointer, value); break;
            default: g_s2e->getDebugStream(m_state) << "LuaS2EExecutionStateRegisters::write: Incorrect size " << size << "\n"; break;
        }
    }
    return 0;
}
