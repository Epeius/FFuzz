...
void HostFiles::initialize()
{
    m_allowWrite = s2e()->getConfig()->getBool(
                getConfigKey() + ".allowWrite", false, NULL);

    ConfigFile::string_list dirs = s2e()->getConfig()->getStringList(
                getConfigKey() + ".baseDirs");
    ...
    s2e()->getCorePlugin()->onStateFork.connect(
            sigc::mem_fun(*this, &HostFiles::onStateFork));
...
void HostFiles::read(S2EExecutionState *state)
{
    ...
    if(guestFd > plgState->m_openFiles.size() ||
    ...
    if(plgState->m_openFiles[guestFd].type != READ){
        getWarningsStream(state)
            << "ERROR: file passed to HostFiles cannot be read (bad permissions)\n";
    }
...
void HostFiles::create(S2EExecutionState *state)
{
    ...
    if(fname.find("/") != std::string::npos) {
        getWarningsStream(state)
                << "HostFiles: file name must not contain '/' characters ("
                << fname << ")\n;" ;
        return;
    }
    ...
    if(!llvm::sys::fs::exists(path)) {
        ...
        if (err) {
             getWarningsStream(state)
                << "HostFiles: couldn't create directory ("
                << path.str() << "), err : " << err.value() << "\n;" ;
        }
    }

    if (!llvm::sys::fs::is_directory(path)) {
        getWarningsStream(state)
                << "HostFiles: couldn't create directory, file exists ("
                << path.str() << ")\n;" ;
        return;
    }
    ...
    if (llvm::sys::fs::exists(path.str())) {
        getWarningsStream(state)
                << "HostFiles: file exists, cannot create ("
                << fname << ")\n;" ;
        return;
    }
...
void HostFiles::write(S2EExecutionState *state)
{
    ...
    if(plgState->m_openFiles[guestFd].type != WRITE){
        getWarningsStream(state)
            << "ERROR: file passed to HostFiles cannot be written to"
            << "(bad permissions)\n";
    }
    ...
    if(!ok) {
        getWarningsStream(state)
            << "ERROR: HostFiles can not read guest buffer\n";
        return;
    }
...
bool HostFiles::addDirectories(const std::string & newdir)
{
    if (!llvm::sys::fs::exists(newdir)) {
        s2e()->getWarningsStream() << "Path " << newdir << " doesn't exist!" << "\n";
        return false;
    }
    m_baseDirectories.push_back(newdir);
    return true;
}
            ...
            const std::vector<klee::ref<klee::Expr> >& newConditions)
{
    ...
    if(plgState->nb_open > 0){
        getWarningsStream(state)
                << "HostFiles : Forking new state with "
