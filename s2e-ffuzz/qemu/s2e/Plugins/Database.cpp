...
void Database::initialize()
{
    ...
    for (auto& database : databases) {
        std::string connection_string =
                cfg->getString(getConfigKey() + '.' + database);

        soci::session& db = m_sessions.emplace(
                    std::piecewise_construct, std::forward_as_tuple(database),
                    std::forward_as_tuple(connection_string)).first->second;
        db << "SET AUTOCOMMIT TO ON";  // FIXME: make this configurable
    }

    s2e()->getCorePlugin()->onProcessFork.connect(sigc::mem_fun(
            *this, &Database::onProcessFork));
}
...
std::string Database::escapeBytea(soci::session &session,
                                  const std::vector<uint8_t>& data) {
    if (session.get_backend_name() == "postgresql") {
        PGconn *conn = static_cast<soci::postgresql_session_backend*>(
                    session.get_backend())->conn_;

	size_t out_length = 0;
	unsigned char *out_bytes = PQescapeByteaConn(
		    conn, data.data(), data.size(), &out_length);
	...
	PQfreemem(out_bytes);
        ...
        llvm::errs() << "Database::escapeBytea() is not supported on "
                  << session.get_backend_name() << " yet\n";
        abort();
    }
}
