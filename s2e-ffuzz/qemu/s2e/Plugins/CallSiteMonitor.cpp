...
S2E_DEFINE_PLUGIN(CallSiteMonitor, "CallSiteMonitor S2E plugin", "", "Interceptor", "ProcessExecutionDetector", "ModuleExecutionDetector");
...
void CallSiteMonitor::initialize()
{
    ...
    if (m_dumpPeriod > 0) {
        s2e()->getCorePlugin()->onTimer.connect(
                sigc::mem_fun(*this, &CallSiteMonitor::onTimer));
    }
                                       ...
                                       uint64_t pc, bool isStatic, uint64_t staticTarget)
{
    ...
    bool isIndirect = tb->se_tb_type == TB_CALL_IND ||
                      tb->se_tb_type == TB_JMP_IND ||
                      tb->se_tb_type == TB_COND_JMP_IND;
...
void CallSiteMonitor::onInstruction(S2EExecutionState *state, uint64_t source_pc,
