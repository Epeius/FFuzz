...
public:
    ...
    iterator begin() { return m_callSites.begin(); }
    iterator end() { return m_callSites.end(); }

    FunctionCall(FunctionCall *parent, const std::string &module,
                 uint64_t function, const std::string &functionName):
    m_parent(parent), m_module(module), m_function(function), m_functionName(functionName),
    m_thingsCount(0), m_selectionCount(0)
    {
 ...
 */
template <typename T>
class CallTreeRandomPath : public CallTreeVisitor<T> {
public:
 ...
 */
template <typename T>
class CallTreeDotPrinter : public CallTreeVisitor<T> {
private:
    typedef typename FunctionCall<T>::Things ThingsT;
...
public:
    CallTreeDotPrinter(llvm::raw_ostream &os) : m_os(os),
    m_totalSelectionCount(0) {
    ...
    void visit(const FunctionCall<T> &call) {
        ...
        m_os << '_' << &call << " [label=\""
           << call.getModule() << ":" << hexval(call.getFunction()) << "\\n";
        ...
        foreach2(it, call.getThings().begin(), call.getThings().end()) {
            ...
            m_os << '_' << &call << '_' << location.second << " [label=\""
 ...
 */
template <typename T>
class CallTreeTextPrinter : public CallTreeVisitor<T> {
private:
    typedef typename FunctionCall<T>::Things ThingsT;
