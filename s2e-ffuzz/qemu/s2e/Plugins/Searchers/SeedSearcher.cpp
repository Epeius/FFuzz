...
void SeedSearcher::onStateSplit(klee::StateSet &parent,
                                klee::StateSet &child)
{
    if (m_initialState) {
...
void SeedSearcher::onStateFork(
        S2EExecutionState *oldState,
...
void SeedSearcher::fetchNewSeeds()
{
    ...
    for (llvm::sys::fs::directory_iterator i(m_seedDirectory, error), e; i != e; i.increment(error)) {
        ...
        if (count < 5) {
            getDebugStream() << "name: " << seed.filename
                             << " index:" << seed.index
...
void SeedSearcher::handleOpcodeInvocation(S2EExecutionState *state,
                                    ...
                                    uint64_t guestDataSize)
{
    ...
    if (guestDataSize != sizeof(command)) {
        getWarningsStream(state) <<
                "S2E_SEEDSEARCHER_COMMAND: mismatched command structure size " << guestDataSize << "\n";
        exit(-1);
    }

    if (!state->mem()->readMemoryConcrete(guestDataPtr, &command, guestDataSize)) {
        getWarningsStream(state) <<
                "S2E_SEEDSEARCHER_COMMAND: could not read transmitted data\n";
        exit(-1);
    }
