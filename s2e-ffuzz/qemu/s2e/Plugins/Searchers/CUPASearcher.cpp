...
S2E_DEFINE_PLUGIN(CUPASearcher, "CUPA searcher", "", "ModuleExecutionDetector", "FunctionMonitor", "MultiSearcher", "ProcessExecutionDetector");
...
klee::Searcher *CUPASearcher::createSearcher(unsigned level)
{
    ...
    if (level < m_classes.size()) {
        switch(m_classes[level]) {
            case SEED: searcher = new CUPASearcherSeedClass(this, level); break;
            case BATCH: searcher = new CUPASearcherBatchClass(this, level); break;
            case PC: searcher = new CUPASearcherPcClass(this, level); break;
            case PAGEDIR: searcher = new CUPASearcherPageDirClass(this, level); break;
            case FORKCOUNT: searcher = new CUPASearcherForkCountClass(this, level); break;
            case PRIORITY: searcher = new CUPASearcherPriorityClass(this, level); break;
            case READCOUNT: searcher = new CUPASearcherReadCountClass(this, level); break;
            case RANDOM: searcher = new CUPASearcherRandomClass(this, level); break;
            case VULNERABILITY: searcher = new CUPAVulnerabilitySearcherClass(this, level); break;
            case GROUP: searcher = new CUPASearcherGroupClass(this, level); break;
...
void CUPASearcher::update(klee::ExecutionState *current,
...
void CUPASearcherClass::doAddState(klee::ExecutionState* state,
                              uint64_t stateClass) {
    auto searchersIt = m_searchers.find(stateClass);
    if (searchersIt == m_searchers.end()) {
        getDebugStream() << "Creating new searcher for class "
                                << hexval(stateClass) << "\n";
        klee::Searcher *searcher = m_plg->createSearcher(m_level + 1);
        searchersIt = m_searchers.emplace(
            stateClass, std::unique_ptr<klee::Searcher>(searcher)).first;
    }
...
void CUPASearcherClass::doRemoveState(klee::ExecutionState *state) {
    ...
    if (searchersIt->second->empty()) {
        getDebugStream() << " class " << hexval(stateClass)
                         << " is empty, deleting its searcher\n";
        m_searchers.erase(searchersIt);
    }
}

void CUPASearcherClass::update(
...
CUPASearcherPriorityClass::CUPASearcherPriorityClass(CUPASearcher *plugin, unsigned level) : CUPASearcherClass(plugin, level)
{
    ...
    if (!s_pc.s_priority_connection.connected()) {
        s_pc.cupa = plugin;
        s_pc.s_priority_connection = m_kvs->onLocalPut.connect(sigc::mem_fun(
                                             s_pc, &PriorityClass::onLocalPut));
    }
}
...
CUPASearcherGroupClass::CUPASearcherGroupClass(CUPASearcher *plugin, unsigned level) : CUPASearcherClass(plugin, level)
{
    m_kvs = plugin->s2e()->getPlugin<KeyValueStore>();
    if (!m_kvs) {
    ...
    if (!s_gc.s_group_connection.connected()) {
        s_gc.cupa = plugin;
        s_gc.s_group_connection = m_kvs->onLocalPut.connect(sigc::mem_fun(
                                             s_gc, &GroupClass::onLocalPut));
    }
}
...
void CUPASearcherRandomClass::update(klee::ExecutionState *current,
                    const klee::StateSet &addedStates,
                    const klee::StateSet &removedStates)
{
...
CUPASearcherReadCountClass::CUPASearcherReadCountClass(CUPASearcher *plugin, unsigned level) : CUPASearcherClass(plugin, level)
{
    m_monitor = static_cast<CGCMonitor*>(plugin->s2e()->getPlugin("CGCMonitor"));

    if (!s_read_conn.connected()) {
        s_read_conn = m_monitor->onSymbolicRead.connect(
            sigc::ptr_fun(&CUPASearcherReadCountClass::onSymbolicRead)
        );
    }
 ...
 */
void CUPASearcherReadCountClass::onSymbolicRead(S2EExecutionState* state, uint64_t pid, uint64_t fd, uint64_t size,
...
CUPAVulnerabilitySearcherClass::CUPAVulnerabilitySearcherClass(CUPASearcher *plugin, unsigned level) : CUPASearcherClass(plugin, level)
{
    m_cfg = NULL;
    m_detector = NULL;
...
void CUPAVulnerabilitySearcherClass::initialize()
{
    ...
    translate_ins_conn = core->onTranslateInstructionStart.connect( sigc::mem_fun(*this, &CUPAVulnerabilitySearcherClass::onTranslateInstruction));
    mod_load_conn = m_detector->onModuleLoad.connect(sigc::mem_fun(*this, &CUPAVulnerabilitySearcherClass::onModuleLoad));
    tb_end_conn = core->onTranslateBlockEnd.connect( sigc::mem_fun(*this, &CUPAVulnerabilitySearcherClass::onTranslateBlockEnd));
    ...
    m_useParentSearcherProbability = config->getInt(m_plg->getConfigKey() + ".vulnerability.useParentSearcherProbability", 0);
    ...
    foreach2(it, keys.begin(), keys.end())
    {
        ...
        ss0 << m_plg->getConfigKey() << ".vulnerability.modules" << "." << keyName;
...
#if 0
        const std::string fileName = "/tmp/" + moduleConfig.moduleName + ".dot";
        ...
        for (int i = 0; i < bbList.size(); i++) {
            const ControlFlowGraph::BasicBlock *bb = bbList[i];

            std::string funcName;
            if (m_cfg->getFunctionName(moduleConfig.moduleName, bb->start_pc, funcName)) {
                file << " \""
                     << hexval(bb->start_pc)
                     << "\" [label=\""
                     << hexval(bb->start_pc)
            ...
            if (bb->call_target) {
                std::string funcName = "???";
                m_cfg->getFunctionName(moduleConfig.moduleName, bb->call_target, funcName);
                file << " \""
                     << hexval(bb->start_pc)
                     << "_call"
                     << "\" [label=\""
                     << hexval(bb->call_target)
                     << "\n"
                     << funcName
                     << "\"]\n";
                file << " \"" << hexval(bb->start_pc) << "\" -> \"" << hexval(bb->start_pc) << "_call" << "\"\n";
            }

            for (int j = 0; j < bb->successors.size(); j++) {
                file << " \""
                     << hexval(bb->start_pc)
                     << (bb->call_target ? "_call" : "")
                     << "\" -> \""
                     << hexval(bb->successors[j])
                     << "\"\n";
            }
        }
        ...
        m_plg->getWarningsStream() << "CFG saved to " << fileName << "\n";
#endif
    }
}
...
void CUPAVulnerabilitySearcherClass::onTranslateBlockEnd(ExecutionSignal *signal, S2EExecutionState *state, TranslationBlock *tb,
        uint64_t pc, bool hasStaticTarget, uint64_t staticPc)
{
...
#if 0
        for (int v = 0; v < module.vulnPcs.size(); v++) {
            m_plg->getWarningsStream(newStates[i]) << hexval(nextPc[i]) << " -> " << hexval(module.vulnPcs[v]) << ": "
            << distances[v] << "\n";
        }
#endif
        ...
        const klee::StateSet &removedStates)
{
    ...
    if (addedStates.size()) {
        foreach2(it, m_states.begin(), m_states.end())
        {
            ...
            llvm::raw_ostream &s = m_plg->getWarningsStream(*it);

            s << "distances: ";
            for (int i = 0; i < distances.size(); i++) {
                s << distances[i] << (i != distances.size() - 1 ? ", " : "");
            }
            s << "\n";
        }
    }
