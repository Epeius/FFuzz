...
PathSearcher::PathSearcher(S2E* s2e) :
        Plugin(s2e)
{
    m_chosenState = NULL;
    m_vulnPc = 0;
...
void PathSearcher::initialize()
{
    ...
    s2e()->getPlugin<CGCMonitor>()->onSegFault.connect(sigc::mem_fun(*this, &PathSearcher::onSegFault));
...
#define FORKED_STATE_ID(fi)     ( (fi)->parentStateId == (fi)->childStateId[0] ? (fi)->childStateId[1] : (fi)->childStateId[0] )
