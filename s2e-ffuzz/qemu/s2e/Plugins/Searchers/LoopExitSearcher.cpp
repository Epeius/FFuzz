...
S2E_DEFINE_PLUGIN(LoopExitSearcher, "Searcher to be used with state merging",
                  "LoopExitSearcher", "LoopDetector", "ControlFlowGraph", "BasicBlockCoverage",
                  "EdgeCoverage");
...
void LoopExitSearcher::initialize()
{
    ...
    s2e()->getCorePlugin()->onStateFork.connect(
            sigc::mem_fun(*this, &LoopExitSearcher::onFork));

    s2e()->getCorePlugin()->onTimer.connect(
            sigc::mem_fun(*this, &LoopExitSearcher::onTimer));
...
void LoopExitSearcher::onTimer()
{
    ...
    m_timerTicks = 0;
    getDebugStream() << "Main queue size: " << m_states.size()
                     << " wait queue size: " << m_waitingStates.size()
                     << "\n";
}

void LoopExitSearcher::onFork(S2EExecutionState *state,
            ...
            )
{
    ...
    for (unsigned i = 0; i < 2; ++i) {
        ...
        os << "Target[" << i << "]:"
           << " state: " << newStates[i]->getID()
           << " dest:" << hexval(targets[i])
           << " finalsucc: " << hexval(finalSucc)
           << " isExit: " << (int) isExit
...
void LoopExitSearcher::increasePriority(S2EExecutionState *state, int64_t priority)
{
    ...
    for (unsigned i = 0; i < 2; ++i) {
        ...
        getDebugStream(state) << "Increasing priority of state " << state->getID() << " by " << priority <<  " new: " << p.priority << "\n";
...
void LoopExitSearcher::update(klee::ExecutionState *current,
