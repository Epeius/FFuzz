...
#include <s2e/Plugins/WindowsInterceptor/WindowsInterceptor.h>
...
class WindowsMonitor2 : public WindowsInterceptor, public BaseInstructionsPluginInvokerInterface
{
    S2E_PLUGIN
public:
    ...
    WindowsMonitor2(S2E* s2e): WindowsInterceptor(s2e) {}
    ...
    sigc::signal<void, S2EExecutionState*,
                 const S2E_WINMON2_ACCESS_FAULT &> onAccessFault;

    sigc::signal<void, S2EExecutionState*,
                 const S2E_WINMON2_ALLOCATE_VM&
                 > onNtAllocateVirtualMemory;

    sigc::signal<void, S2EExecutionState*,
                 const S2E_WINMON2_FREE_VM&
                 > onNtFreeVirtualMemory;

    sigc::signal<void, S2EExecutionState*,
                 const S2E_WINMON2_PROTECT_VM&
                 > onNtProtectVirtualMemory;

    sigc::signal<void, S2EExecutionState*,
                 const S2E_WINMON2_MAP_SECTION&
                 > onNtMapViewOfSection;

    sigc::signal<void, S2EExecutionState*,
                 const S2E_WINMON2_UNMAP_SECTION&
                 > onNtUnmapViewOfSection;
    ...
    struct MemoryInformation {
        ...
        MemoryInformation() {
            CommitCharge = VirtualSize = PeakVirtualSize =
                    PeakCommitCharge = 0;
        }
    };
...
private:
    ...
    void opcodeInitKernelStructs(S2EExecutionState *state,
                                  uint64_t guestDataPtr,
                                  const S2E_WINMON2_COMMAND &command);

    void loadUnloadOpcode(S2EExecutionState *state,
                           uint64_t guestDataPtr,
                           const S2E_WINMON2_COMMAND &command);
...
public:
    ...
    bool getVirtualMemoryInfo(S2EExecutionState *state, uint64_t Process, uint64_t Address,
