...
private:
    bool patchImportsFromDisk(S2EExecutionState *state,
                              const ModuleDescriptor &module,
                              uint32_t checkSum, vmi::Imports &imports);

    template <typename T>
    void getContext(S2EExecutionState *state, T &context);
...
public:
    ...
    template<typename T>
    static bool readConcreteParameter(S2EExecutionState *s, unsigned param, T *val) {
