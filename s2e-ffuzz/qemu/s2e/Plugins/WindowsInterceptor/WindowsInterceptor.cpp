...
bool WindowsInterceptor::patchImportsFromDisk(S2EExecutionState *state,
        ...
        for (vmi::ImportedSymbols::iterator fit = symbols.begin(); fit != symbols.end(); ++fit) {
            ...
            if (!state->readPointer(itl, address)) {
                getWarningsStream(state) <<
                    "could not read address " << hexval(itl) << "\n";
                continue;
            }
...
template <typename T>
void WindowsInterceptor::getContext(S2EExecutionState *state, T &Context)
{
