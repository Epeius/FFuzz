...
S2E_DEFINE_PLUGIN(BlueScreenInterceptor, "Intercepts Windows blue screens of death and generated bug reports",
                  "BlueScreenInterceptor", "Interceptor");
...
void BlueScreenInterceptor::onBsod(S2EExecutionState *state, uint64_t pc)
{
    ...
    ss << "BSOD: code=" << hexval(code)
            << " param1=" << hexval(param1)
            << " param2=" << hexval(param2)
            << " param3=" << hexval(param3)
            << " param4=" << hexval(param4);


    s2e()->getExecutor()->terminateStateEarly(*state, ss.str());
}

void BlueScreenInterceptor::handleOpcodeInvocation(S2EExecutionState *state,
                                    ...
                                    uint64_t guestDataSize)
{
    ...
    if (guestDataSize != sizeof(command)) {
        getWarningsStream(state) <<
                "BlueScreenInterceptor: mismatched S2E_BSOD_COMMAND size\n";
        return;
    }

    if (!state->mem()->readMemoryConcrete(guestDataPtr, &command, guestDataSize)) {
        getWarningsStream(state) <<
                "BlueScreenInterceptor: could not read transmitted data\n";
        return;
    }
    ...
    std::stringstream ss;
    ss << "BSOD: code=" << hexval(command.Code)
            << " param1=" << hexval(command.Parameters[0])
            << " param2=" << hexval(command.Parameters[1])
            << " param3=" << hexval(command.Parameters[2])
