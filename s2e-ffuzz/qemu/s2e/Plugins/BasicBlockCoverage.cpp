...
void BasicBlockCoverage::initialize()
{
    ...
    s2e()->getCorePlugin()->onTimer.connect(
            sigc::mem_fun(*this, &BasicBlockCoverage::onTimer));

    s2e()->getCorePlugin()->onUpdateStates.connect(
            sigc::mem_fun(*this, &BasicBlockCoverage::onUpdateStates));

    m_cfg->onReload.connect(
            sigc::mem_fun(*this, &BasicBlockCoverage::onCfgReload));
    ...
    if (writeCoverageOnStateKill) {
        s2e()->getCorePlugin()->onStateKill.connect(
                sigc::mem_fun(*this, &BasicBlockCoverage::onStateKill));
    }
...
void BasicBlockCoverage::onUpdateStates(S2EExecutionState* state,
        ...
        foreach2(bit, bypc.begin(), bypc.end()) {
            ...
            if (filter.find(state) != filter.end()) {
                getDebugStream() << "BasicBlockCoverage: "
                        << "State id " << (*bit).state->getID()
                        << " has not covered bb " << hexval((*bit).pc) << " yet\n";
                return (*bit).state;
            }
        }
    }
...
void BasicBlockCoverage::onModuleTranslateBlockComplete(S2EExecutionState *state,
