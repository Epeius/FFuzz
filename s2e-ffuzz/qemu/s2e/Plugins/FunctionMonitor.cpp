...
void FunctionMonitor::slotModuleTranslateBlockEnd(ExecutionSignal *signal,
                                      S2EExecutionState *state,
...
void FunctionMonitor::slotTraceCall(S2EExecutionState *state, FunctionMonitorState *fns)
{
    ...
    getInfoStream(state) << "Calling function " << f
...
{
    getInfoStream(state) << "Returning from function "
                << f << std::endl;
}
#endif
