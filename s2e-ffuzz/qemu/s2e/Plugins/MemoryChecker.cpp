    ...
    if(nextModule) {
        m_dataMemoryAccessConnection =
            s2e()->getCorePlugin()->onAfterSymbolicDataMemoryAccess.connect(
                sigc::mem_fun(*this, &MemoryChecker::onAfterSymbolicDataMemoryAccess)
            );
    } else {
...
void MemoryChecker::onAfterSymbolicDataMemoryAccess(S2EExecutionState *state,
                                       ...
                                       unsigned flags)
{
    if (state->isRunningExceptionEmulationCode()) {
        ...
        //getWarningsStream() << "Running emulation code" << std::endl;
        return;
    }

    if(!isa<klee::ConstantExpr>(virtualAddress)) {
        getWarningsStream(state) << "Symbolic memory accesses are "
    ...
    if (m_traceMemoryAccesses) {
        m_memoryTracer->traceSymbolicDataMemoryAccess(state, virtualAddress, hostAddress, value, flags);
    }
    ...
    onPreCheck.emit(state, start, accessSize, flags & MEM_TRACE_FLAG_WRITE);

    std::string errstr;
    llvm::raw_string_ostream err(errstr);
    bool result = checkMemoryAccess(state, start,
                      accessSize,
                      (flags & MEM_TRACE_FLAG_WRITE) ? 2 : 1, err);


    if (!result) {
        onPostCheck.emit(state, start, accessSize, flags & MEM_TRACE_FLAG_WRITE, &result);
        if (result) {
            return;
        }
        ...
        if(m_terminateOnErrors)
            ...
            getWarningsStream(state) << err.str();
    }
}
...
//    getDebugStream() << "matchRegionType typePrefix=" << typePrefix
                                ...
                                bool permanent)
{
    ...
    getDebugStream(state) << "MemoryChecker::grantMemory("
            << *region << ")" << '\n';
    ...
    if(size == 0 || start + size < start) {
        getWarningsStream(state) << "MemoryChecker::grantMemory: "
                                 ...
                                 const std::string &regionTypePattern, uint64_t regionID)
{
    ...
    getDebugStream(state) << "MemoryChecker::revokeMemory("
            << *region << ")" << '\n';
                   ...
                   uint64_t handle, const std::string &resourceType)
{
    ...
    getDebugStream(state) << "MemoryChecker::grantResource(" << res << ")\n";
    ...
    if (exres) {
        getWarningsStream(state) << "MemoryChecker::grantResource: "
...
void MemoryChecker::revokeResource(S2EExecutionState *state,  uint64_t handle)
{
    ...
    getDebugStream(state) << "MemoryChecker::revokeResource("
...
bool MemoryChecker::checkResourceLeaks(S2EExecutionState *state)
{
    ...
    getDebugStream(state) << "MemoryChecker::checkResourceLeaks" << '\n';
...
bool MemoryChecker::checkMemoryLeaks(S2EExecutionState *state)
{
    ...
    getDebugStream(state) << "MemoryChecker::checkMemoryLeaks" << '\n';
