...
S2E_DEFINE_PLUGIN(LuaFunctionAnnotation, "LuaFunctionAnnotation S2E plugin", "LuaFunctionAnnotation",
                  "ModuleExecutionDetector", "FunctionMonitor", "Interceptor", "LuaBindings");
...
bool LuaFunctionAnnotation::registerAnnotation(const Annotation &annotation)
{
    ...
    getDebugStream() << "LuaFunctionAnnotation: loaded "
                            << annotation.moduleId << " "
...
void LuaFunctionAnnotation::initialize()
{
    ...
    m_monitor->onModuleLoad.connect(
                sigc::mem_fun(*this,
                              &LuaFunctionAnnotation::onModuleLoad));

    m_monitor->onModuleUnload.connect(
                sigc::mem_fun(*this,
                              &LuaFunctionAnnotation::onModuleUnload));
}

void LuaFunctionAnnotation::hookAnnotation(S2EExecutionState *state, const ModuleDescriptor &module, const Annotation *annotation)
{
...
void LuaFunctionAnnotation::onModuleLoad(
...
void LuaFunctionAnnotation::onModuleUnload(
        S2EExecutionState* state,
        const ModuleDescriptor &module
        )
{
...
void LuaFunctionAnnotation::onFunctionCall(
        S2EExecutionState* state,
...
void LuaFunctionAnnotation::handleOpcodeInvocation(S2EExecutionState *state,
                                                   ...
                                                   uint64_t guestDataSize)
{
    ...
    if (guestDataSize != sizeof(command)) {
        getWarningsStream(state) <<
                "LuaFunctionAnnotation: mismatched S2E_LUA_FCN_ANN_COMMAND size\n";
        return;
    }

    if (!state->mem()->readMemoryConcrete(guestDataPtr, &command, guestDataSize)) {
        getWarningsStream(state) <<
                "LuaFunctionAnnotation: could not read transmitted data\n";
        return;
    }
    ...
    switch (command.Command) {
        ...
        default: {
            getWarningsStream(state) <<
                    "LuaFunctionAnnotation: incorrect command " << command.Command << "\n";
        }
    }
