...
S2E_DEFINE_PLUGIN(LoopDetector, "LoopDetector S2E plugin", "", "EdgeDetector", "ModuleExecutionDetector", "ControlFlowGraph");
...
void LoopDetector::initialize()
{
    ...
    foreach2(it, keys.begin(), keys.end()) {
        ...
        foreach2(eit, loop_keys.begin(), loop_keys.end()) {
            ...
            #if 0
            /* Compute the edges that enter the loop */
            /* Note: this does not work if the bb right before the loop has a call instruction */
            const ControlFlowGraph::BasicBlock *bb, *predbb;
            bb = m_cfg->findBasicBlock(moduleConfig.moduleName, header);
            if (bb) {
                foreach2(pit, bb->predecessors.begin(), bb->predecessors.end()) {
                    predbb = m_cfg->findBasicBlock(moduleConfig.moduleName, *pit);
                    assert(predbb);
                    m_edgeDetector->addEdge(moduleConfig.moduleName, predbb->end_pc, header, EdgeDetector::EDGE_LOOP_ENTRY);
                }
            }
            #endif
        }
    ...
    if (m_headers.size() > 0 || m_exitBlocks.size() > 0) {
        ...
        m_edgeDetector->onEdge.connect(
            sigc::mem_fun(*this, &LoopDetector::onEdge)
        );
    }
}
...
//to make it generic.
void LoopDetector::onModuleTransition(S2EExecutionState *state,
...
void LoopDetector::onModuleTranslateBlockStart(
        ...
        uint64_t pc)
{
    ...
    m_ins_connection = s2e()->getCorePlugin()->onTranslateInstructionStart.connect(
        sigc::bind(
            sigc::mem_fun(*this, &LoopDetector::onTranslateInstructionEnd),
 ...
 */
void LoopDetector::onTranslateInstructionEnd(ExecutionSignal *signal, S2EExecutionState *state,
                                             TranslationBlock *tb,
                                             ...
                                             LoopExitBlocks *_exitblocks)
{
...
#if 0
    LoopExitBlocks::iterator eit = (*_exitblocks).find(modulePc);
    if (eit != _exitblocks->end()) {
        getDebugStream(state) << "LoopDetector: translation detected exit block at " << hexval(modulePc) << "\n";
        signal->connect(
            sigc::mem_fun(*this, &LoopDetector::onLoopExit)
        );
    }
#endif
    ...
    if (hit != _headers->end()) {
        ...
        signal->connect(
            sigc::mem_fun(*this, &LoopDetector::onLoopHeader)
        );
    }
}
...
void LoopDetector::onModuleTranslateBlockComplete(
        S2EExecutionState* state,
        const ModuleDescriptor &module,
        TranslationBlock *tb,
        uint64_t endPc)
