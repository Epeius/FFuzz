...
S2E_DEFINE_PLUGIN(GuestCodePatching, "Transparent patching of guest code", "GuestCodePatching",
                  "Interceptor", "ModuleExecutionDetector");

void GuestCodePatching::initialize()
{
    m_windowsMonitor = dynamic_cast<WindowsInterceptor*>(s2e()->getPlugin("Interceptor"));
    if (!m_windowsMonitor) {
    ...
    m_windowsMonitor->onModuleLoad.connect(
            sigc::mem_fun(*this,
                    &GuestCodePatching::onModuleLoad)
            );

    m_windowsMonitor->onModuleUnload.connect(
            sigc::mem_fun(*this,
                    &GuestCodePatching::onModuleUnload)
            );
    ...
    m_detector->onModuleTranslateBlockEnd.connect(
        sigc::mem_fun(*this, &GuestCodePatching::onModuleTranslateBlockEnd)
    );

    s2e()->getCorePlugin()->onTimer.connect(
            sigc::mem_fun(*this, &GuestCodePatching::onTimer));
}
...
void GuestCodePatching::onModuleLoad(
...
void GuestCodePatching::onModuleUnload(
        S2EExecutionState* state,
        const ModuleDescriptor &module
        )
{
...
//////////////////////////////////////////////////////
void GuestCodePatching::onModuleTranslateBlockStart(
        ...
        uint64_t pc)
{
    ...
    if (hookAddress) {
        ...
        signal->connect_front(
            sigc::bind(sigc::mem_fun(*this, &GuestCodePatching::onExecuteBlockStart), true, plgState->getDiectKernelHookAddress(pc))
        );
    }
}


void GuestCodePatching::invokeHook(S2EExecutionState *state, uint64_t pc, uint64_t hookAddress, uint64_t returnAddress)
{
    ...
    getDebugStream(state) << "GuestCodePatching: "
                                 << "jumping to address " << hexval(hookAddress)
                                 << " on stack " << hexval(stackBase) << " size=" << hexval(stackSize)
                                 << "\n";
...
#if 0
    getDebugStream(state) << "GuestCodePatching: "
                                 << hexval(ra) << " is in " << md->Name << "\n";
#endif

    //Prevent infinite recursion.
    //Takes into account lfidriver.sys and lfidriver64.sys
    if (md->Name.find("lfidriver") != std::string::npos) {
        return;
    }
...
//////////////////////////////////////////////////////
void GuestCodePatching::onModuleTranslateBlockEnd(
        ExecutionSignal *signal,
        S2EExecutionState* state,
        const ModuleDescriptor &module,
        TranslationBlock *tb,
        uint64_t pc, bool staticTarget, uint64_t targetPc)
{
    ...
    signal->connect(
        sigc::mem_fun(*this, &GuestCodePatching::onExecuteCall)
    );
}
...
bool GuestCodePatching::patchImports(S2EExecutionState *state, const ModuleDescriptor &module)
{
    ...
    if (!guestImage) {
        getWarningsStream(state) <<
                "GuestCodePatching: error creating GuestMemoryFileProvider\n";
        return false;
    }
    ...
    if (!pe) {
        getWarningsStream(state) <<
                "GuestCodePatching: could not load memory image for "
                << module.Name << "\n";
        delete guestImage;
        return false;
    }
    ...
    if (pe->getPointerSize() > state->getPointerSize()) {
        getWarningsStream(state) <<
                "GuestCodePatching: image pointer size " << pe->getPointerSize()
    ...
    for (vmi::Imports::const_iterator it = imports.begin(); it != imports.end(); ++it) {
        ...
        for (vmi::ImportedSymbols::const_iterator fit = symbols.begin(); fit != symbols.end(); ++fit) {
            ...
            ss << std::setfill(' ') << std::setw(40) << std::left << symbolName
                    << " @0x" << std::hex << address
                    << " @0x" << std::hex << itl;
                    ...
                    if (state->readPointer(itl, originalAddress)) {
                        if (originalAddress == address) {
                            if (!state->writePointer(itl, hookAddress)) {
                                getWarningsStream(state) <<
                                    "GuestCodePatching: could not write hook location\n";
                                ss << " Hook failed";
                            }
                        } else {
                            getWarningsStream(state) <<
                                "GuestCodePatching: IAT address mismatch\n";
                            ss << " Hook failed";
                        }
                    } else {
                        getWarningsStream(state) <<
                            "GuestCodePatching: could not read hook location\n";
                        ss << " Hook failed";
                    }
                }
            }
...
void GuestCodePatching::opcodePatchExistingModule(S2EExecutionState *state,
                                                  ...
                                                  const S2E_HOOK_PLUGIN_COMMAND &command)
{
    ...
    if (!ret) {
        getWarningsStream(state) <<
                "GuestCodePatching: could not read module and/or function name\n";
        goto err1;
    }
    ...
    if (!module) {
        getWarningsStream(state) <<
                "GuestCodePatching: could not find a descriptor for module " << ModuleName << "\n";
        goto err1;
    }

    if (!patchImports(state, *module)) {
        getWarningsStream(state) <<
                "GuestCodePatching: could not patch imports in module " << ModuleName << "\n";
        goto err1;
    }
...
void GuestCodePatching::opcodeRegisterKernelFunction(S2EExecutionState *state,
                                                     const S2E_HOOK_PLUGIN_COMMAND &command)
{
    bool ret = true;
    ...
    getDebugStream(state) << "GuestCodePatching: "
            << "Registering hook for " << ModuleName << "!" << FunctionName
            << " using function @" << hexval(command.KernelFunction.Address)
            << "\n";

    m_registeredHooks[ModuleName][FunctionName] = command.KernelFunction.Address;
}

void GuestCodePatching::opcodeRegisterEntryPoint(S2EExecutionState *state,
                                                        uint64_t guestDataPtr,
                                                        const S2E_HOOK_PLUGIN_COMMAND &command)
{
    ...
    if (!address) {
        ...
        getDebugStream(state) << "GuestCodePatching: "
                << "setting DriverEntry hook "
                << hexval(command.EntryPoint.Hook) << "\n";
    ...
    if (!module) {
        getDebugStream(state) << "GuestCodePatching: "
                << "opcodeRegisterEntryPoint could not get module for entry point "
                << hexval(address) << "\n";
        return;
    }
    ...
    getDebugStream(state) << "GuestCodePatching: registering entry point "
            << ep.Name << "@" << hexval(ep.Address) << " in module " << module->Name
            << " hook @" << hexval(ep.Hook) << " Handle @" << hexval(ep.Handle) << "\n";
...
//TODO: have a signal for that
#if 0
        /* For the load balancer */
        EntryPoint mep;
        mep.ModuleAddress = module->ToNativeBase(address);
        mep.Name = ep.Name;
        m_entryPoints[module->Name].insert(mep);
#endif
    }
}

void GuestCodePatching::handleOpcodeInvocation(S2EExecutionState *state,
                                    uint64_t guestDataPtr,
                                    uint64_t guestDataSize)
{
    ...
    if (guestDataSize != sizeof(command)) {
        getWarningsStream(state) <<
                "GuestCodePatching: mismatched S2E_HOOK_PLUGIN_COMMAND size\n";
        return;
    }

    if (!state->mem()->readMemoryConcrete(guestDataPtr, &command, guestDataSize)) {
        getWarningsStream(state) <<
                "GuestCodePatching: could not read transmitted data\n";
        return;
    }
    ...
    switch (command.Command) {
        ...
        case HOOK_MODULE_IMPORTS: {
            ...
            getDebugStream(state) <<
                "GuestCodePatching: flushing TB cache\n";
            tb_flush(env);
        ...
        case DEREGISTER_DIRECT_KERNEL_HOOK: {
            ...
            if (plgState->isDirectKernelHook(command.DirectHook.HookedFunctionPc)) {
                ...
                getDebugStream(state) << "GuestCodePatching: "
                                             << hexval(command.DirectHook.HookedFunctionPc)  << " was not hooked\n";
            }
        } break;
    }
}
...
void GuestCodePatchingState::deregisterAllEntryPoints(uint64_t handle, bool external)
{
    ...
    foreach2(it, entryPoints.begin(), entryPoints.end()) {
        ...
        foreach2(it2, toErase.begin(), toErase.end()) {
            eps.erase(*it2);
        }
    }
}

uint64_t GuestCodePatchingState::getHookAddress(const ModuleDescriptor &module, uint64_t original, bool external) const
{
    if (!external) {
