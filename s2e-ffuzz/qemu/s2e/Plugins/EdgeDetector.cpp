...
namespace {
    ...
    llvm::cl::opt<bool>
    DebugEdgeDetector("s2e-debug-edge-detector",
                     llvm::cl::init(false));
}

S2E_DEFINE_PLUGIN(EdgeDetector, "Fires an event when a specified sequence of instructions has been executed", "EdgeDetector",
...
void EdgeDetector::initialize()
{
    ...
    m_detector->onModuleTranslateBlockStart.connect(
        sigc::mem_fun(*this, &EdgeDetector::onModuleTranslateBlockStart)
    );

    m_detector->onModuleTransition.connect(
        sigc::mem_fun(*this,
                &EdgeDetector::onModuleTransition)
    );
}
...
void EdgeDetector::readConfig(const std::string &configKey, IEdgeAdder *adder)
{
    ...
    foreach2(it, keys.begin(), keys.end()) {
        ...
        foreach2(eit, edge_keys.begin(), edge_keys.end()) {
            ...
            getDebugStream() << "EdgeDetector: "
                    << moduleConfig.moduleName << ": "
                    << hexval(il[0]) << " => " << hexval(il[1]) << "\n";
...
{
    getDebugStream() << "EdgeDetector: adding edge " << moduleName
                            << " " << hexval(start) << " => " << hexval(end) << " type:" << type << "\n";
...
//////////////////////////////////////////////////////
void EdgeDetector::onModuleTransition(S2EExecutionState *state,
...
void EdgeDetector::onModuleTranslateBlockStart(
        ...
        uint64_t pc)
{
    ...
    m_ins_connection = s2e()->getCorePlugin()->onTranslateInstructionEnd.connect(
        sigc::bind(
...
void EdgeDetector::onTranslateInstructionEnd(ExecutionSignal *signal, S2EExecutionState *state,
                                             TranslationBlock *tb,
                                             uint64_t pc, uint64_t addend, const EdgeCollection::Edges *_edges)
{
    ...
    signal->connect(
        sigc::bind(
            sigc::mem_fun(*this, &EdgeDetector::onEdgeInternal),
            addend,
            &(*it).second
        )
    );
    ...
    foreach2(it, targets.begin(), targets.end()) {
        ...
        if (moduleDestPc == target.first) {
            if (DebugEdgeDetector) {
                getInfoStream(state) << "EdgeDetector: "
                        << hexval(sourcePc) << " => " << hexval(state->getPc()) << "\n";
            }
...
void EdgeDetector::onModuleTranslateBlockComplete(
        S2EExecutionState* state,
        const ModuleDescriptor &module,
        TranslationBlock *tb,
        uint64_t endPc
