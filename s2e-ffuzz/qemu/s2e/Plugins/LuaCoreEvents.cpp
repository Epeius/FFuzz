...
std::string LuaCoreEvents::checkCoreSignal(const std::string &cfgname,
                                           const std::string &name)
{
    ...
    if (ret.length() > 0) {
        if (!cfg->isFunctionDefined(ret)) {
            getWarningsStream()
                    << ret << " is not declared in the Lua script\n";
            exit(-1);
        }
...
void LuaCoreEvents::registerCoreSignals(const std::string &cfgname)
{
    ...
    if (m_onStateKill.length() > 0) {
        s2e()->getCorePlugin()->onStateKill.connect(
                sigc::mem_fun(*this, &LuaCoreEvents::onStateKill)
        );
    }
    ...
    if (m_onTimer.length() > 0) {
        s2e()->getCorePlugin()->onTimer.connect(
                sigc::mem_fun(*this, &LuaCoreEvents::onTimer)
        );
    }
}
