...
S2E_DEFINE_PLUGIN(UserSpaceTracer, "Traces user-space processes", "", "ExecutionTracer", "Interceptor");

void UserSpaceTracer::initialize()
{
    m_monitor = static_cast<OSMonitor*>(s2e()->getPlugin("Interceptor"));
    m_winmonitor = s2e()->getPlugin<WindowsMonitor2>();
    ...
    if (m_traceTranslation) {
        if (m_monitor) {
            m_monitor->onModuleLoad.connect(
                sigc::mem_fun(*this, &UserSpaceTracer::onModuleLoad)
            );
        }
...
void UserSpaceTracer::onTranslateBlockComplete(
            S2EExecutionState* state,
            TranslationBlock *tb,
            uint64_t endPc)
{
...
void UserSpaceTracer::onTranslateBlockStart(
        ...
        uint64_t pc)
{
    ...
    signal->connect(
        sigc::mem_fun(*this, &UserSpaceTracer::onExecuteBlockStart)
    );
}
