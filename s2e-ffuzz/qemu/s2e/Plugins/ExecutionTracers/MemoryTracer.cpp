...
void MemoryTracer::initialize()
{
    ...
    getDebugStream() << "MonitorMemory: " << m_monitorMemory <<
...
void MemoryTracer::traceConcreteDataMemoryAccess(S2EExecutionState *state,
                               uint64_t address,
                               uint64_t value,
                               uint8_t size,
                               unsigned flags)
...
void MemoryTracer::traceSymbolicDataMemoryAccess(S2EExecutionState *state,
...
void MemoryTracer::onAfterSymbolicDataMemoryAccess(S2EExecutionState *state,
                               klee::ref<klee::Expr> address,
                               klee::ref<klee::Expr> hostAddress,
                               klee::ref<klee::Expr> value,
                               unsigned flags)
...
void MemoryTracer::onConcreteDataMemoryAccess(S2EExecutionState *state,
                               uint64_t address,
                               uint64_t value,
                               uint8_t size,
                               unsigned flags)
...
void MemoryTracer::onModuleTranslateBlockStart(
...
{
    signal->connect(
        sigc::mem_fun(*this, &MemoryTracer::onExecuteBlockStart)
    );
}
    ...
    if (m_monitorMemory) {
        ...
        if (m_monitorModules) {
            ...
            m_execDetector->onModuleTranslateBlockStart.connect(
                    sigc::mem_fun(*this,
                            &MemoryTracer::onModuleTranslateBlockStart)
                    );
        } else {
            connectMemoryTracing();
        }
