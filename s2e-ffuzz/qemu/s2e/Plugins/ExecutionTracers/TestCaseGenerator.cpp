...
{
    m_connection = s2e()->getCorePlugin()->onStateKill.connect(
            sigc::mem_fun(*this, &TestCaseGenerator::onStateKill));
}
...
void TestCaseGenerator::onStateKill(S2EExecutionState *state)
{
    ...
    foreach2(it, state.constraints.begin(), state.constraints.end()) {
        getInfoStream() << "Constraint: " << std::hex << *it << '\n';
    }
#endif
...
llvm::raw_ostream *TestCaseGenerator::getTestCaseFile(S2EExecutionState *state)
{
    ...
    if (!out) {
        getWarningsStream(state)
                << "TestCaseGenerator: could not open file " << testCaseName.str() << '\n';
        return NULL;
    }
...
bool TestCaseGenerator::getFilePart(const std::string &variableName, std::string &filePath,
                                    unsigned *part, unsigned *numberOfParts) const
{
    /** Find the file name */
    const std::string startMarker = "__symfile$$$";
...
void TestCaseGenerator::getFiles(const ConcreteInputs &inputs,
                                 TestCaseFiles &files)
{
...
bool TestCaseGenerator::generateFile(S2EExecutionState *state, const std::string &filePath, TestCaseFile &file)
{
    if (file.totalParts != file.chunks.size()) {
        getWarningsStream()
                << "Test case for " << filePath << " has incorrect number of parts\n";
        return false;
    }

    if (file.chunksData.size() != file.chunks.size()) {
        getWarningsStream()
                << "Test case for " << filePath << " has not enough data chunks\n";
        return false;
    }
...
{
    getInfoStream()
            << "TestCaseGenerator: writing test case of state " << state->getID() << '\n';
    ...
    if (!success) {
        getWarningsStream()
                << "TestCaseGenerator: Could not get symbolic solutions for state " << state->getID() << '\n';
        return;
    }
