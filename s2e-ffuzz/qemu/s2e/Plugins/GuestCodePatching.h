...
private:
    ...
    void opcodePatchExistingModule(S2EExecutionState *state,
                                   uint64_t guestDataPtr,
                                   const S2E_HOOK_PLUGIN_COMMAND &command);

    void opcodeRegisterEntryPoint(S2EExecutionState *state,
                                  uint64_t guestDataPtr,
                                  const S2E_HOOK_PLUGIN_COMMAND &command);

    void opcodeRegisterKernelFunction(S2EExecutionState *state,
                                      const S2E_HOOK_PLUGIN_COMMAND &command);
