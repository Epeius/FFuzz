...
void ModuleExecutionDetector::handleOpcodeGetModule(S2EExecutionState *state,
...
void ModuleExecutionDetector::handleOpcodeInvocation(S2EExecutionState *state,
                                    uint64_t guestDataPtr,
                                    uint64_t guestDataSize)
{
    ...
    if (guestDataSize != sizeof(command)) {
        getWarningsStream(state) <<
                "ModuleExecutionDetector: mismatched S2E_MODEX_DETECTOR_COMMAND size\n";
        return;
    }

    if (!state->mem()->readMemoryConcrete(guestDataPtr, &command, guestDataSize)) {
        getWarningsStream(state) <<
                "ModuleExecutionDetector: could not read transmitted data\n";
        return;
    }
...
void ModuleExecutionDetector::processUnloadListener(
    S2EExecutionState* state, uint64_t addressSpace, uint64_t pid)
{
    DECLARE_PLUGINSTATE(ModuleTransitionState, state);
...
const ModuleDescriptor *ModuleExecutionDetector::getModule(S2EExecutionState *state, uint64_t pc, bool tracked)
{
    ...
    const ModuleDescriptor *currentModule =
            plgState->getDescriptor(addressSpace, pc, tracked);
    return currentModule;
}

const ModuleDescriptor *ModuleExecutionDetector::getModule(S2EExecutionState *state, uint64_t addressSpace, uint64_t pc, bool tracked)
{
    DECLARE_PLUGINSTATE(ModuleTransitionState, state);
...
const ModuleDescriptor *ModuleExecutionDetector::getModule(S2EExecutionState *state, const std::string &moduleName, bool tracked)
{
    DECLARE_PLUGINSTATE(ModuleTransitionState, state);
    const ModuleDescriptor *currentModule =
            plgState->getDescriptor(moduleName, tracked);
    return currentModule;
}
...
std::vector<const ModuleDescriptor*> ModuleExecutionDetector::getModules(S2EExecutionState *state, uint64_t addressSpace, bool tracked)
{
    DECLARE_PLUGINSTATE(ModuleTransitionState, state);
    ...
    uint64_t pc)
{
    ...
    const ModuleDescriptor *currentModule =
            plgState->getDescriptor(addressSpace, pc);
...
void ModuleExecutionDetector::onTranslateBlockComplete(S2EExecutionState *state,
                                                       TranslationBlock *tb,
                                                       uint64_t pc)
{
...
klee::ref<klee::Expr> ModuleExecutionDetector::readMemory(S2EExecutionState *state, uint64_t addr, klee::Expr::Width width)
{
