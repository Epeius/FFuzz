...
{
    out << "ModuleDescriptor Name=" << md.Name  <<
      " Path=" << md.Path <<
      " NativeBase=" << hexval(md.NativeBase) << " LoadBase=" << hexval(md.LoadBase) <<
      " Size=" << hexval(md.Size) <<
      " AddressSpace=" << hexval(md.AddressSpace) <<
      " Pid=" << hexval(md.Pid) <<
      " EntryPoint=" << hexval(md.EntryPoint) <<
      " Checksum=" << hexval(md.Checksum);
