...
S2E_DEFINE_PLUGIN(ProcessExecutionDetector, "ProcessExecutionDetector S2E plugin", "", "Interceptor");
...
void ProcessExecutionDetector::initialize()
{
    ...
    m_monitor->onProcessLoad.connect(
            sigc::mem_fun(*this,
                    &ProcessExecutionDetector::onProcessLoad)
            );

    m_monitor->onProcessUnload.connect(
            sigc::mem_fun(*this,
                    &ProcessExecutionDetector::onProcessUnload)
            );

    m_monitor->onMonitorLoad.connect(
            sigc::mem_fun(*this,
                    &ProcessExecutionDetector::onMonitorLoadCb)
            );
}


void ProcessExecutionDetector::onProcessLoad(S2EExecutionState *state, uint64_t pageDir, uint64_t pid, const std::string &ImageFileName)
{
