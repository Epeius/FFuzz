...
void ResourceMonitor::initialize()
{
    s2e()->getCorePlugin()->onStateForkDecide.connect(
            sigc::mem_fun(*this, &ResourceMonitor::onStateForkDecide));

    s2e()->getCorePlugin()->onTimer.connect(
            sigc::mem_fun(*this, &ResourceMonitor::onTimer));
...
void ResourceMonitor::dropStates()
{
    ...
    if (nrStatesToTerminate > 0) {
        ...
        getDebugStream() << "ResourceMonitor: will kill "
                                << nrStatesToTerminate
                                << " states, reason: out of memory"
                                << " #states = " << executor->getStatesCount() << "\n";
        getDebugStream()
                << "ResourceMonitor: limit - rss (MB) = "
                << ((m_cgroupMemLimit - m_rss) / 1024/1024)  << "\n";

        for (auto it = states.begin();
             it != states.end() && nrStatesToTerminate > 0;
             it++, nrStatesToTerminate--)
        {
