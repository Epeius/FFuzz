...
S2E_DEFINE_PLUGIN(ModuleMap, "Tracks loaded modules", "", "Interceptor");
...
public:
    ...
    void onModuleUnload(const ModuleDescriptor &module) {
        ...
        if ((*it).LoadBase != module.LoadBase) {
            g_s2e->getDebugStream(g_s2e_state)
                    << "ModuleMap::onModuleUnload mismatched base addresses:\n"
                    << "  looked for:" << module << "\n"
                    << "  found     :" << *it << "\n";
    ...
    void dump(llvm::raw_ostream &os) const {
        ...
        foreach2(it, m_modules.begin(), m_modules.end()) {
            os << *it << "\n";
        }
        os << "==========================================\n";
    }
...
void ModuleMap::initialize()
{
    ...
    m_monitor->onModuleLoad.connect(
            sigc::mem_fun(*this,
                    &ModuleMap::onModuleLoad)
            );

    m_monitor->onModuleUnload.connect(
            sigc::mem_fun(*this,
                    &ModuleMap::onModuleUnload)
            );

    WindowsMonitor2 *winmon2 = dynamic_cast<WindowsMonitor2 *>(m_monitor);
    if (winmon2) {
        winmon2->onMonitorLoad.connect(
            sigc::mem_fun(*this,
                    &ModuleMap::onMonitorLoad)
        );
    }
}
...
void ModuleMap::onMonitorLoad(S2EExecutionState *state)
{
    ...
    if (!winmon2->moduleUnloadSupported()) {
        ...
        winmon2->onNtUnmapViewOfSection.connect(
            sigc::mem_fun(*this,
                    &ModuleMap::onNtUnmapViewOfSection)
        );
    }
}
...
void ModuleMap::onProcessUnload(
    S2EExecutionState* state,
