...
template<typename T>
static inline bool s2e_fast_read_mem(uint64_t addr, T *res)
{
...
#ifdef CONFIG_LIBS2E
        //XXX: fix this hack
        #define ldl_p(x) g_sqi.libcpu.ldl_p(x)
        #define ldq_p(x) g_sqi.libcpu.ldq_p(x)
#endif

        if (sizeof(*res) == 4) {
            *res = ldl_p((uint8_t *)(addr + addend));
        } else if (sizeof(*res) == 8) {
            *res = ldq_p((uint8_t *)(addr + addend));
        } else {
...
template<typename T>
static void *s2e_expr_read_mem(void *_mgr, uint64_t virtual_address)
{
    ExprManager *mgr = static_cast<ExprManager*>(_mgr);
    ExprBox *retbox = mgr->create();
