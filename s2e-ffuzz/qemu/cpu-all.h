...
static inline int _se_check_concrete(void *objectState,
...
#define _s2e_define_ld_raw(ct, t, s) \
    static inline ct ld ## t ## _raw(const void* p) { \
        ...
        uint8_t buf[s]; \
        s2e_read_ram_concrete((uint64_t) p, buf, s); \
    ...
    static inline ct ld ## t ## _raw_symb(const void* p) { \
        ...
        uint8_t buf[s]; \
        s2e_read_ram_concrete_check((uint64_t) p, buf, s); \
        return ld ## t ## _p(buf); \
    }
...
#define _s2e_define_st_raw(ct, t, s) \
    static inline void st ## t ## _raw(void* p, ct v) { \
        ...
        uint8_t buf[s]; \
        st ## t ## _p(buf, v); \
        s2e_write_ram_concrete((uint64_t) p, buf, s); \
    }\
...
void list_mapped_pages(CPUX86State *env, unsigned rw_only, unsigned user_only, target_ulong **pages_addr, size_t *pages_count);
