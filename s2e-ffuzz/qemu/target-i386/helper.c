...
{
#ifdef CONFIG_SYMBEX
    if (unlikely(*g_s2e_on_page_directory_change_signals_count)) {
        ...
        size_t *pages_count)
{
    ...
    target_ulong *list = malloc(allocated * sizeof(target_ulong));

    assert(!(env->cr[4] & CR4_PAE_MASK) && "PAE is not supported");
    assert((env->cr[0] & CR0_PG_MASK) && "Paging is not enabled");

    for (unsigned pde_n = 0; pde_n < 1024; pde_n++) {
        target_ulong pde_addr = ((env->cr[3] & ~0xfff) + (pde_n << 2)) & env->a20_mask;
        uint32_t pde = ldl_phys(pde_addr);

        if (!(pde & PG_PRESENT_MASK)) {
            continue;
        }

        if (user_only && !(pde & PG_USER_MASK)) {
            continue;
        }

        assert(!((pde & PG_PSE_MASK) && (env->cr[4] & CR4_PSE_MASK)) && "Big pages are not supported");

        for (unsigned pte_n = 0; pte_n < 1024; pte_n++) {
            target_ulong pte_addr = ((pde & ~0xfff) + (pte_n << 2)) & env->a20_mask;
            uint32_t pte = ldl_phys(pte_addr);

            if (!(pte & PG_PRESENT_MASK)) {
                continue;
            }

            if (rw_only && !(pte & PG_RW_MASK)) {
                continue;
            }
            if (user_only && !(pte & PG_USER_MASK)) {
                continue;
            }

            target_ulong addr = (pde_n << 22) + (pte_n << 12);

            if (count + 1 > allocated) {
                allocated *= 2;
                list = realloc(list, allocated * sizeof(target_ulong));
            }
