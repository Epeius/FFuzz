...
static inline target_ulong __RR_env_raw(CPUX86State* cpuState,
    ...
    if (likely(g_s2e_fast_concrete_invocation)) {
        switch (size) {
            case 1: return *((uint8_t*)cpuState + offset);
            case 2: return *(uint16_t*)((uint8_t*)cpuState + offset);
            case 4: return *(uint32_t*)((uint8_t*)cpuState + offset);
            case 8: return *(uint64_t*)((uint8_t*)cpuState + offset);
            default: assert(false); return 0;
        }
    } else {
        target_ulong result = 0;
        s2e_read_register_concrete(offset, (uint8_t*) &result, size);
        return result;
    }
}
static inline void __WR_env_raw(CPUX86State* cpuState, unsigned offset,
    ...
    if (likely(g_s2e_fast_concrete_invocation)) {
        switch (size) {
            case 1: *((uint8_t*)cpuState + offset) = value; break;
            case 2: *(uint16_t*)((uint8_t*)cpuState + offset) = value; break;
            case 4: *(uint32_t*)((uint8_t*)cpuState + offset) = value; break;
            case 8: *(uint64_t*)((uint8_t*)cpuState + offset) = value; break;
            default: assert(false);
        }
    } else {
        s2e_write_register_concrete(offset, (uint8_t*) &value, size);
    }
}
...
#ifdef S2E_ENABLE_PRECISE_EXCEPTION_DEBUGGING
#define WR_s2eeip(cpu, value) cpu->s2e_eip = value
#else
#define WR_s2eeip(cpu, value)
#endif
...
{
    WR_s2eeip(env, tb->pc - tb->cs_base);
