...
#if defined(CONFIG_SYMBEX) && !defined(STATIC_TRANSLATOR)
    if (unlikely(*g_s2e_on_privilege_change_signals_count)) {
        s2e_on_privilege_change(cpl, dpl);
    }
#endif
...
{
#if defined(CONFIG_SYMBEX) && !defined(STATIC_TRANSLATOR)
    if (s2e_is_port_symbolic(port)) {
        s2e_switch_to_symbolic(GETPC());
    }
    if (*g_s2e_on_port_access_signals_count)
        s2e_trace_port_access(port, data, 8, 1, GETPC());
#endif
    cpu_outb(port, data & 0xff);
}
...
#if defined(CONFIG_SYMBEX) && !defined(STATIC_TRANSLATOR)
    if (*g_s2e_on_port_access_signals_count)
        s2e_trace_port_access(port, res, 8, 0, GETPC());
#endif
    return res;
}
...
void helper_outw(uint32_t port, uint32_t data)
{
    ...
    if (*g_s2e_on_port_access_signals_count)
        s2e_trace_port_access(port, data, 16, 1, GETPC());
#endif
    cpu_outw(port, data & 0xffff);
}
...
#if defined(CONFIG_SYMBEX) && !defined(STATIC_TRANSLATOR)
    if (*g_s2e_on_port_access_signals_count)
        s2e_trace_port_access(port, res, 16, 0, GETPC());
#endif
    return res;
}
...
void helper_outl(uint32_t port, uint32_t data)
{
    ...
    if (*g_s2e_on_port_access_signals_count)
        s2e_trace_port_access(port, data, 32, 1, GETPC());
#endif
    cpu_outl(port, data);
}
...
#if defined(CONFIG_SYMBEX) && !defined(STATIC_TRANSLATOR)
    if (*g_s2e_on_port_access_signals_count)
        s2e_trace_port_access(port, res, 32, 0, GETPC());
#endif
    return res;
}
...
{\
    val = s2e_expr_read_mem_q(mgr, sp);\
    sp += 8;\
}
#endif
...
void helper_syscall(int next_eip_addend)
{
    ...
    if (env->hflags & HF_LMA_MASK) {
        ...
        if (code64) {
            env->eip = env->lstar;
            WR_s2eeip(env, env->lstar);
        } else {
            env->eip = env->cstar;
            WR_s2eeip(env, env->cstar);
        }
    } else {
        ...
        WR_s2eeip(env, (uint32_t)env->star);
    }

#if defined(CONFIG_SYMBEX) && !defined(STATIC_TRANSLATOR)
    if (unlikely(*g_s2e_on_privilege_change_signals_count)) {
        s2e_on_privilege_change(old_cpl, 0);
    }
#endif
}
#endif
...
#if defined(CONFIG_SYMBEX) && !defined(STATIC_TRANSLATOR)
    if (unlikely(*g_s2e_on_privilege_change_signals_count)) {
        s2e_on_privilege_change(old_cpl, 3);
    }
#endif
}
#endif
...
extern se_do_interrupt_all_t do_interrupt_all;
...
static inline void helper_ret_protected(int shift, int is_iret, int addend)
{
...
#if defined(CONFIG_SYMBEX) && !defined(STATIC_TRANSLATOR)
    if (cpl != rpl && unlikely(*g_s2e_on_privilege_change_signals_count)) {
        s2e_on_privilege_change(cpl, rpl);
    }
#endif
 ...
 return_to_vm86:
    ...
    #if defined(CONFIG_SYMBEX) && !defined(SYMBEX_LLVM_LIB) && !defined(STATIC_TRANSLATOR)
    se_load_eflags(mgr, symb_new_eflags, TF_MASK | AC_MASK | ID_MASK |
...
#if !defined(CONFIG_SYMBEX) || (defined(CONFIG_SYMBEX) && !defined(SYMBEX_LLVM_LIB)) || defined(STATIC_TRANSLATOR)
...
#if !defined(CONFIG_SYMBEX) || (defined(CONFIG_SYMBEX) && !defined(SYMBEX_LLVM_LIB))
...
#if !defined(CONFIG_USER_ONLY) && !defined(STATIC_TRANSLATOR)
              ...
              void *retaddr)
{
    ...
    if (unlikely(*g_s2e_on_tlb_miss_signals_count)) {
        s2e_on_tlb_miss(addr, is_write, retaddr);
    }
#endif
...
#if defined(CONFIG_SYMBEX)
        if (unlikely(*g_s2e_on_page_fault_signals_count)) {
            s2e_on_page_fault(addr, is_write, retaddr);
        }
#endif
...
void helper_register_symbols()
{
    ...
    helper_register_symbol("helper_iret_protected", helper_iret_protected);
}
#endif

#if defined(CONFIG_SYMBEX)
/* These are dummy functions, to be redefined by the S2E plugin */
...
extern int g_savevm_take_snapshot;
extern int g_savevm_snapshot_taken;
extern const int g_savevm_name_max_len;
extern char g_savevm_snapshot_name[];
...
void helper_se_opcode(uint64_t opcode)
{
    ...
    if (opc != VANILLA_QEMU_OPCODE) {
        return;
    }

    uint8_t function = (opcode >> 16) & 0xFF;
    if (function == VANILLA_QEMU_OPCODE_SAVEVM) {
        if (g_savevm_take_snapshot) {
            return;
        }

        int count = 0;
        target_ulong name = EAX;

        memset(g_savevm_snapshot_name, 0, sizeof(g_savevm_name_max_len));
        do {
            g_savevm_snapshot_name[count] = ldub_kernel(name);
            ++name;
            ++count;
        } while ((count < g_savevm_name_max_len-1) && g_savevm_snapshot_name[count - 1]);

        g_savevm_take_snapshot = 1;
    } else if (function == VANILLA_QEMU_OPCODE_SAVEVM_QUERY) {
        if (!g_savevm_take_snapshot) {
            EAX_W(-1);
            return;
        }

        if (g_savevm_snapshot_taken) {
            EAX_W(-3);
            return;
        }
