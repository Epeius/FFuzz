...
#define s2e_on_translate_indirect_cti_start(...)
#endif
...
static inline void s2e_gen_call_ret(DisasContext *s, int isCall)
{
    if (likely(!g_s2e_on_call_return_signals_count)) {
        return;
    }
    ...
    int instrument = s2e_on_call_return_translate(s->pc, isCall);
    if (!instrument) {
        return;
    }
...
{
    if (unlikely(*g_s2e_on_translate_instruction_end_signals_count && s->instrument))
        s2e_on_translate_instruction_end(s, s->tb, s->insPc, s->useNextPc ? s->nextPc : (uint64_t)-1);
}

static inline void gen_s2e_eob_event(DisasContext *s, int static_target, target_ulong target_pc)
{
    gen_instr_end(s);

    if (unlikely(*g_s2e_on_translate_register_access_signals_count && s->instrument))
        s2e_translate_compute_reg_mask_end(s);

    if (unlikely(*g_s2e_on_translate_block_end_signals_count && s->instrument))
        s2e_on_translate_block_end(s->tb, s->insPc, static_target, target_pc);

    if (unlikely(*g_s2e_on_call_return_signals_count && s->instrument)) {
        if (s->tb->se_tb_type == TB_CALL || s->tb->se_tb_type == TB_CALL_IND) {
            s2e_gen_call_ret(s, 1);
        } else if (s->tb->se_tb_type == TB_RET) {
            s2e_gen_call_ret(s, 0);
        }
    }
...
static inline void gen_op_jmp_T0(DisasContext *s)
{
    ...
    tcg_gen_st_tl(cpu_T[0], cpu_env, offsetof(CPUX86State, s2e_eip));
    #endif
}
...
static inline void gen_jmp_im(DisasContext *s, target_ulong pc)
{
    ...
    #ifdef S2E_ENABLE_PRECISE_EXCEPTION_DEBUGGING
    tcg_gen_st_tl(cpu_tmp0, cpu_env, offsetof(CPUX86State, s2e_eip));
    #endif
}
...
#ifdef CONFIG_SYMBEX
                    if (unlikely(
                        *g_s2e_on_translate_lea_rip_relative_signals_count && s->instrument)) {
                        s2e_on_translate_lea_rip_relative(s, s->tb,
                                                          s->tb->pcOfLastInstr,
                                                          disp);
                    }
#endif
                           ...
                           target_ulong val, target_ulong next_eip)
{
    ...
    if (s->jmp_opt) {
        ...
        #ifdef CONFIG_SYMBEX
        gen_s2e_eob_event(s, 1, next_eip);
        #endif
        tcg_gen_br(l2);
...
static void gen_eob(DisasContext *s)
{
     ...
     * Do it before calling the S2E event, to avoid deadlocks
     * if the event exits the cpu loop.
     */
...
#ifdef CONFIG_SYMBEX
/**
 * S2E plugins can do anything with the state.
 * Ensure here that it is consistent. In particular,
 * flags must be fully computed to allow plugins
...
static target_ulong disas_insn(DisasContext *s, target_ulong pc_start)
{
    ...
    tcg_gen_st_tl(cpu_tmp0, cpu_env, offsetof(CPUArchState, s2e_eip));
#endif

#ifdef CONFIG_SYMBEX
    //XXX: Incrementing the instruction count at every instruction makes things
    //very slow.
 ...
 reswitch:
    ...
    case 0x13f: /* s2e_op */
        {
#ifdef CONFIG_SYMBEX
            if (likely(g_s2e_allow_custom_instructions)) {
                s2e_gen_pc_update(s, pc_start, s->cs_base);

                uint64_t arg = ldq_code(s->pc);
                s2e_tcg_emit_custom_instruction(arg);
            } else {
                goto illegal_op;
            }
#else
            /* Handle opcodes that are specific to vanilla QEMU */
...
#ifdef CONFIG_SYMBEX
                if (s->instrument) {
                    s2e_on_translate_indirect_cti_start(s, s->tb, s->insPc, rm, op, offset_addr);
                }
#endif
                gen_op_ld_T0_A0(ot + s->mem_index);
            }
    ...
    case 0x6a:
        ...
        #ifdef CONFIG_SYMBEX
        if (unlikely(*g_s2e_on_translate_special_instruction_end_signals_count)) {
            s2e_on_translate_special_instruction_end(s, s->tb, pc_start, PUSHIM, 0);
        }
        #endif
    ...
    case 0xc2: /* ret im */
        SET_TB_TYPE(TB_RET);
#ifdef CONFIG_SYMBEX
        if (unlikely(*g_s2e_on_translate_jump_start_signals_count && s->instrument)) {
            s2e_on_translate_jump_start(s, s->tb, pc_start, JT_RET);
        }
#endif
        val = ldsw_code(s->pc);
        s->pc += 2;
...
#ifdef CONFIG_SYMBEX
        if (unlikely(*g_s2e_on_translate_jump_start_signals_count && s->instrument)) {
            s2e_on_translate_jump_start(s, s->tb, pc_start, JT_LRET);
        }
#endif
        val = ldsw_code(s->pc);
        s->pc += 2;
    ...
    case 0xcc: /* int3 */
#ifdef CONFIG_SYMBEX
        if (unlikely(*g_s2e_on_translate_soft_interrupt_signals_count && s->instrument)) {
            s2e_on_translate_soft_interrupt_start(s, s->tb, pc_start, 3);
        }
#endif
...
#ifdef CONFIG_SYMBEX
        if (unlikely(*g_s2e_on_translate_soft_interrupt_signals_count && s->instrument)) {
            s2e_on_translate_soft_interrupt_start(s, s->tb, pc_start, val);
        }
#endif
    ...
    case 0x131: /* rdtsc */
        ...
        #ifdef CONFIG_SYMBEX
        if (unlikely(*g_s2e_on_translate_special_instruction_end_signals_count)) {
            s2e_on_translate_special_instruction_end(s, s->tb, pc_start, RDTSC, 0);
        }
        #endif
    ...
    case 0x134: /* sysenter */
        ...
        if (!s->pe) {
            ...
            #ifdef CONFIG_SYMBEX
            if (unlikely(*g_s2e_on_translate_special_instruction_end_signals_count)) {
                s2e_on_translate_special_instruction_end(s, s->tb, pc_start, SYSENTER, 0);
            }
            #endif
            gen_eob(s);
        }
    ...
    case 0x105: /* syscall */
        ...
        #ifdef CONFIG_SYMBEX
        if (unlikely(*g_s2e_on_translate_special_instruction_end_signals_count)) {
            s2e_on_translate_special_instruction_end(s, s->tb, pc_start, SYSCALL, 0);
        }
        #endif
        gen_eob(s);
        break;
                                                  ...
                                                  int search_pc)
{
    ...
    if (unlikely(*g_s2e_on_translate_block_start_signals_count && dc->instrument)) {
        s2e_on_translate_block_start(dc, tb, pc_start);
    }
    #endif
#endif
    ...
    for(;;) {
        ...
        if (unlikely(*g_s2e_on_translate_instruction_start_signals_count && dc->instrument)) {
            s2e_on_translate_instruction_start(dc, tb, pc_ptr);
        }
        ...
        if (unlikely(*g_s2e_on_translate_register_access_signals_count && dc->instrument))
        ...
        if (dc->tf || dc->singlestep_enabled ||
            ...
            #ifdef CONFIG_SYMBEX
            gen_s2e_eob_event(dc, 1, pc_ptr - dc->cs_base);
            #endif
            gen_eob(dc);
            break;
        }
...
#ifdef CONFIG_SYMBEX
        if (unlikely(*g_s2e_on_translate_block_complete_signals_count && dc->instrument)) {
            s2e_on_translate_block_complete(tb, dc->insPc);
        }
#endif
    }
}
    ...
    if (qemu_loglevel_mask(CPU_LOG_TB_OP)) {
        ...
        for(i = 0;i <= pc_pos; i++) {
            if (gen_opc_instr_start[i]) {
                qemu_log("0x%04x: " TARGET_FMT_lx " cc_op=%d\n", i, gen_opc_pc[i], gen_opc_cc_op[i]);
            }
        }
