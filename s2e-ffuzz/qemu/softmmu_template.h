...
#include <s2e/s2e_config.h>
    ...
    #define S2E_AFTER_MEMORY_ACCESS(vaddr, value, flags, retaddr) \
    ...
    #define SE_SET_MEM_IO_VADDR(env, addr, reset) \
        tcg_llvm_write_mem_io_vaddr(addr, reset)
            ...
            #define S2E_BEFORE_MEMORY_ACCESS(vaddr, value, flags)
            #define S2E_AFTER_MEMORY_ACCESS(vaddr, value, flags, retaddr) \
                if (unlikely(*g_s2e_after_memory_access_signals_count)) s2e_after_memory_access(vaddr, value, sizeof(value), flags, (uintptr_t) 0);
        #endif
    #else
        #define S2E_BEFORE_MEMORY_ACCESS(vaddr, value, flags)
        #define S2E_AFTER_MEMORY_ACCESS(vaddr, value, flags, retaddr)
                                              ...
                                              void *retaddr)
{
    ...
    if (unlikely(g_s2e_enable_mmio_checks && glue(se_is_mmio_symbolic_, SUFFIX)(mr, mr->addr + physaddr))) {
        s2e_switch_to_symbolic(retaddr);
    }

    if (unlikely(mr == &io_mem_notdirty)) {
...
inline DATA_TYPE glue(io_read_chk_symb_, SUFFIX)(const char *label, MemoryRegion *mr, target_ulong physaddr, uintptr_t pa)
{
    s2e_kill_state("Use fast helpers for symbolic MMIO");
    return 0;
}
                                          ...
                                          void *retaddr)
{
    ...
    char label[64];
    int isSymb = 0;
    //XXX: Cannot dereference mr from here, because it's not registered in KLEE
    if (g_s2e_enable_mmio_checks && (isSymb = glue(se_is_mmio_symbolic_, SUFFIX)(mr, mr->addr + naddr))) {
        s2e_kill_state("Use fast helpers for symbolic MMIO");
    }
    ...
    if (s2e_ismemfunc(mr, 0)) {
        uintptr_t pa = s2e_notdirty_mem_read(naddr & TARGET_PAGE_MASK) | (naddr & (TARGET_PAGE_SIZE - 1));
        if (isSymb) {
            return glue(io_read_chk_symb_, SUFFIX)(ENV_VAR label, mr, naddr, (uintptr_t)(pa));
        }
    ...
    if (s2e_ismemfunc(mr, 0)) {
        uintptr_t pa = s2e_notdirty_mem_read(naddr & TARGET_PAGE_MASK) | (naddr & (TARGET_PAGE_SIZE - 1));
        if (isSymb) {
            res = glue(io_read_chk_symb_, SUFFIX)(label, mr, naddr, (uintptr_t)(pa));
            res |= glue(io_read_chk_symb_, SUFFIX)(label, mr, naddr, (uintptr_t)(pa+4)) << 32;
        }else {
            res = glue(glue(ld, USUFFIX), _raw)((uint8_t *)(intptr_t)(pa));
                                                       ...
                                                       int mmu_idx)
{
    ...
    S2E_BEFORE_MEMORY_ACCESS(addr, 0, 0);
 ...
 redo:
    ...
    if (likely((addr & TARGET_PAGE_MASK) == (tlb_addr & (TARGET_PAGE_MASK | TLB_INVALID_MASK)))) {
        if (unlikely(tlb_addr & ~TARGET_PAGE_MASK)) {
            ...
            S2E_AFTER_MEMORY_ACCESS(addr, res, MEM_TRACE_FLAG_IO, retaddr);
...
#endif
            S2E_AFTER_MEMORY_ACCESS(addr, res, 0, retaddr);
        }
    } else {
                                          ...
                                          void *retaddr)
{
    ...
    if (s2e_ismemfunc(mr, 1)) {
        uintptr_t pa = s2e_notdirty_mem_write(physaddr & TARGET_PAGE_MASK) | (physaddr & (TARGET_PAGE_SIZE - 1));
...
#ifdef CONFIG_SYMBEX_MP
    S2E_BEFORE_MEMORY_ACCESS(addr, val, 1);
    addr = S2E_FORK_AND_CONCRETIZE_ADDR(addr, ADDR_MAX);
    object_index = S2E_FORK_AND_CONCRETIZE(addr >> SE_RAM_OBJECT_BITS,
                                           ADDR_MAX >> SE_RAM_OBJECT_BITS);
    index = (object_index >> S2E_RAM_OBJECT_DIFF) & (CPU_TLB_SIZE - 1);
 ...
 redo:
    ...
    if (likely((addr & TARGET_PAGE_MASK) == (tlb_addr & (TARGET_PAGE_MASK | TLB_INVALID_MASK)))) {
        if (unlikely(tlb_addr & ~TARGET_PAGE_MASK)) {
            ...
            S2E_AFTER_MEMORY_ACCESS(addr, val, MEM_TRACE_FLAG_IO | MEM_TRACE_FLAG_WRITE, retaddr);
        } else if (unlikely(((addr & ~SE_RAM_OBJECT_MASK) + DATA_SIZE - 1) >= SE_RAM_OBJECT_SIZE)) {
...
#endif
            S2E_AFTER_MEMORY_ACCESS(addr, val, MEM_TRACE_FLAG_WRITE, retaddr);
        }
    } else {
        /* the page is not in the TLB : fill it */
#ifndef SYMBEX_LLVM_LIB
