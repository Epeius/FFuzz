...
#ifdef CONFIG_SYMBEX
struct se_qemu_interface_t g_sqi;
#endif
...
void cpu_exec_init_all(void)
{
    ...
    g_sqi.libcpu.ldl_p = ldl_raw;
    g_sqi.libcpu.ldq_p = ldq_raw;
#endif
}
...
static void unassigned_mem_write(target_phys_addr_t addr,
...
static void notdirty_mem_write(target_phys_addr_t ram_addr,
                               uint64_t val, unsigned size)
{
    int dirty_flags;
...
static inline uint32_t ldl_phys_internal(target_phys_addr_t addr)
{
    ...
    if (!sreg) {
        ...
        /* RAM case */
        ptr = qemu_get_ram_ptr((sreg->ram_addr & TARGET_PAGE_MASK)
                               + mem_desc_get_offset(sreg, addr));
        val = ldl_raw(ptr);
    }
...
static inline uint64_t ldq_phys_internal(target_phys_addr_t addr)
{
    ...
    if (!sreg) {
        ...
        /* RAM case */
        ptr = qemu_get_ram_ptr((sreg->ram_addr
                                & TARGET_PAGE_MASK)
                               + mem_desc_get_offset(sreg, addr));
        val = ldq_raw(ptr);
    }
...
void stl_phys_notdirty(target_phys_addr_t addr, uint32_t val)
{
    ...
    if (!sreg) {
        ...
        unsigned long addr1 = (sreg->ram_addr
                               & TARGET_PAGE_MASK)
            + mem_desc_get_offset(sreg, addr);
...
static inline void stl_phys_internal(target_phys_addr_t addr, uint32_t val)
{
    ...
    if (!sreg) {
        ...
        unsigned long addr1;
        addr1 = (sreg->ram_addr & TARGET_PAGE_MASK)
            + mem_desc_get_offset(sreg, addr);
        /* RAM case */
        ptr = qemu_get_ram_ptr(addr1);
        stl_raw(ptr, val);
