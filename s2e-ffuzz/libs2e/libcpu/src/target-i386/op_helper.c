...
static inline void se_load_eflags(void *mgr, void *eflags, int update_mask)
{
    ...
    uint64_t concrete_flags =
            g_sqi.expr.to_constant(
                    g_sqi.expr.andc(mgr, eflags, MFLAGS_MASK & update_mask)
                    );
...
static inline void load_eflags(int eflags, int update_mask)
{
    ...
    */
    ...
    target_ulong old_if = env->mflags & IF_MASK;

    env->mflags = (env->mflags & ~update_mask) |
                    (eflags & MFLAGS_MASK & update_mask);

    if (!old_if && (env->mflags & IF_MASK)) {
        helper_sti();
    }
...
void helper_wrmsr_v(target_ulong index, uint64_t val)
{
    ...
    case MSR_IA32_MISC_ENABLE:
        ...
        break;
    default:
        if ((uint32_t)index >= MSR_MC0_CTL
            && (uint32_t)index < MSR_MC0_CTL + (4 * env->mcg_cap & 0xff)) {
