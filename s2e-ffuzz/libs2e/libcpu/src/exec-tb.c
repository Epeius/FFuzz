...
spinlock_t tb_chain_lock = SPIN_LOCK_UNLOCKED;
...
static void code_gen_alloc(unsigned long tb_size)
{
    ...
    g_code_gen_buffer_max_size = code_gen_buffer_size -
...
#ifdef CONFIG_SYMBEX
    if (g_nb_tbs >= code_gen_max_blocks ||
...
void tb_flush(CPUArchState *env1)
{
    ...
    printf("qemu: flush code_size=%ld g_nb_tbs=%d avg_tb_size=%ld\n",
           (unsigned long)(g_code_gen_ptr - code_gen_buffer),
           g_nb_tbs, g_nb_tbs > 0 ?
           ((unsigned long)(g_code_gen_ptr - code_gen_buffer)) / g_nb_tbs : 0);
#endif
                              ...
                              int flags, int cflags)
{
    ...
    g_code_gen_ptr = (void *)(((unsigned long)g_code_gen_ptr + code_gen_size + CODE_GEN_ALIGN - 1) & ~(CODE_GEN_ALIGN - 1));
...
TranslationBlock *tb_find_pc(uintptr_t tc_ptr)
{
    ...
    if (tc_ptr < (unsigned long)code_gen_buffer ||
        tc_ptr >= (unsigned long)g_code_gen_ptr)
        return NULL;
