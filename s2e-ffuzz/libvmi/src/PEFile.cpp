...
bool PEFile::initImports(void)
{
    ...
    for (unsigned i = 0; i < ImportDescCount; ++i) {
        ...
        do {
            ...
            ImpFcnIt[FunctionName] = ImportedSymbol(AddressEntry.u1.Function,
                                                    m_loadAddress +  ImportAddressTable - sizeof(T));
...
uint64_t PEFile::offset(uint64_t rva) const
{
    ...
    if (m_cachedSection) {
        if (m_cachedSection->VirtualAddress <= rva && rva < m_cachedSection->VirtualAddress + m_cachedSection->Misc.VirtualSize) {
...
bool PEFile::initSections(void)
{
    ...
    uint64_t pSection = m_loadAddress + m_dosHeader.e_lfanew +
...
bool PEFile::initRelocations(void)
{
    ...
    uint8_t *RelocDir =
            static_cast<uint8_t *>(readDirectory(alloc, IMAGE_DIRECTORY_ENTRY_BASERELOC));
    ...
    while (offset < RelocDataDir->Size) {
        ...
        for (unsigned i = 0; i < count; ++i) {
            ...
            //Looks like PE for x86 32-bits and 64-bits has only one type of relocation
...
bool PEFile::initAdditionalFunctionAddresses(void)
{
    ...
    for (it = m_relocations.begin(); it != m_relocations.end(); ++it) {
        ...
        /**
         * Some DLLs do weird stuff with the base address to compute function addresses, e.g.:
                ...
                if (m_file->readb(&ehaddr, sizeof(ehaddr), this->offset(off))) {
                    ...
                    if (ok) {
                        ...
                        while (tbl.Cnt3) {
                            ...
                            /* Only the first one seems to be a function, the others might not be called */
                            break;
                        }
                        ...
                        if (m_file->readb(&sctable, sizeof(sctable), this->offset(off))) {
                            ...
                            for (unsigned i = 0; i < sctable.Count; i++) {
                                m_file->readb(&sctable.ScopeRecord[0], sizeof(sctable.ScopeRecord[0]), this->offset(off));
                                ehaddr = m_imageBase + sctable.ScopeRecord[0].HandlerAddress;

                                if (ehaddr < m_imageBase) {
                                    /* We have invalid data */
                                    break;
                                }
...
bool PEFile32::initialize()
{
    ...
    /*
    if (m_ntHeader.OptionalHeader.FileAlignment != m_ntHeader.OptionalHeader.SectionAlignment) {
        llvm::errs() << m_moduleName << " does not have the same on-disk and in-memory alignment\n";
    }
    */
...
bool PEFile64::initialize()
{
    ...
    /*
    if (m_ntHeader.OptionalHeader.FileAlignment != m_ntHeader.OptionalHeader.SectionAlignment) {
        llvm::errs() << m_moduleName << " does not have the same on-disk (" << m_ntHeader.OptionalHeader.FileAlignment << ")"
                << " and in-memory alignment (" << m_ntHeader.OptionalHeader.SectionAlignment << ")\n";
    }
    */
