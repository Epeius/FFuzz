...
bool WindowsCrashDumpGenerator::writeHeader(const CONTEXT32 &ctx,
                                            const BugCheckDescription &bugDesc)
{
    ...
    bool ok = m_virtualMemory->read(&KdDebuggerDataBlock, sizeof(KdDebuggerDataBlock), hdr->KdDebuggerDataBlock);
    if (!ok) {
        llvm::errs() << "WindowsCrashDumpGenerator: Could not read KD_DEBUGGER_DATA_BLOCK32\n";
        return false;
    }
    ...
    ok = m_virtualMemory->readb(&pMmPhysicalMemoryBlock,
                               sizeof(pMmPhysicalMemoryBlock),
    ...
    uint32_t RunCount;
    ok = m_virtualMemory->read(&RunCount, sizeof(RunCount), pMmPhysicalMemoryBlock);
    ...
    ok = m_virtualMemory->read(&MmPhysicalMemoryBlock[0],
                                   SizeOfMemoryDescriptor,
                                   pMmPhysicalMemoryBlock);
    if (!ok) {
...
bool WindowsCrashDumpGenerator::writeMemoryData(HEADER *Header)
{
    ...
    while (CurrentMemoryRun < ppmd->NumberOfRuns) {
        ...
        while (ProcessedPagesInCurrentRun < ppmd->Run[CurrentMemoryRun].PageCount) {
            ...
            if (!m_physicalMemory->read(tempPage, sizeof(tempPage), physAddr)) {
                llvm::errs() << "WindowsCrashDumpGenerator: could not read physical page "
                        << physAddr << "\n";
            }
...
bool WindowsCrashDumpGenerator::generate(
        uint64_t pKdDebuggerDataBlock,
        uint64_t pKpcrb,
        const DBGKD_GET_VERSION64 &kdVersion,
...
bool WindowsCrashDumpGenerator::generate(const BugCheckDescription &bugDesc,
                                         void *context, unsigned contextSize)
{
