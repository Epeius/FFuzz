...
bool ElfDwarf::getNameAttribute(Dwarf_Die die, const char **name)
{
    ...
    err2: dwarf_dealloc(m_dbg, name_attr, DW_DLA_ATTR);
    err1:
...
bool ElfDwarf::getTypeAttribute(Dwarf_Die die, Dwarf_Die *result_die)
{
    ...
    err2: dwarf_dealloc(m_dbg, attribute, DW_DLA_ATTR);
    err1:
    return retval;
}
    ...
    *result = NULL;
    ...
    switch (tag) {
        ...
        default: {
            ...
            m_errs << "ElfDwarf::buildType: " << "Unhandled tag: " << tag_name << '\n';
            retval = false;
        }
        break;
    }
...
bool ElfDwarf::buildArrayType(Dwarf_Die die, VmiType **result)
{
    ...
    *result = VmiArrayType::get(base_type, elements);
    ...
    err4: dwarf_dealloc(m_dbg, upper_bound_attr, DW_DLA_ATTR);
    err3: dwarf_dealloc(m_dbg, array_desc_die, DW_DLA_DIE);
    err2: dwarf_dealloc(m_dbg, basetype_die, DW_DLA_DIE);
    err1:
    return retval;
}
...
bool ElfDwarf::getMemberOffsetInBytes(Dwarf_Die member_die, unsigned *offset)
{
    ...
    //DW_AT_data_member_location stores the member's offset from the start of the parent structure
    ...
    if (loc_buf[0].ld_s[0].lr_atom != DW_OP_plus_uconst) {
        m_errs << "ElfDwarf::getMemberOffsetInBytes: location atom must be DW_OP_plus_uconst\n";
        goto err3;
    }
    ...
    *offset = loc_buf[0].ld_s[0].lr_number;
    ...
    err3: dwarf_dealloc(m_dbg, loc_buf[0].ld_s, DW_DLA_LOC_BLOCK);
          dwarf_dealloc(m_dbg, loc_buf, DW_DLA_LOCDESC);
    err2: dwarf_dealloc(m_dbg, member_location_attr, DW_DLA_ATTR);
    err1:
    return retval;
}


bool ElfDwarf::buildMemberType(Dwarf_Die member_die,
    ...
    if (res == DW_DLV_OK) {
        do {
            ...
            if (!buildMemberType(kid, member_name, result, isUnion)) {
                m_errs << "ElfDwarf::buildStructure: Could not build member " << member_name << " of structure " << struct_name << '\n';
                break;
            }
...
bool ElfDwarf::processFile(Elf * elf)
{
    ...
    m_elf_max_address = (m_elf_address_size == 8 ) ?
...
ElfDwarf* ElfDwarf::get(
        llvm::raw_ostream &errs,
        const std::string &elfBinary)
{
