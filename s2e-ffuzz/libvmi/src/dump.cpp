...
static void dumpSections(ExecutableFile *file, std::ostream &ss, bool compact)
{
    ...
    if (sections.size() == 0) {
        if (!compact) {
            ss << "#There are no sections in the file\n";
        }
        return;
    }
    ...
    for (Sections::const_iterator it = sections.begin(); it != sections.end(); ++it) {
        ...
        ss << std::setfill(' ') <<  std::setw(20) << std::left << section.name
           << " " << R << W << X
           << " 0x" << std::hex << std::setfill('0') << std::right << std::setw(10) << section.start
           << " 0x" << std::hex << std::setfill('0') << std::right << std::setw(10) << section.size << "\n";
    }
...
static void dumpExports(PEFile *peFile, std::ostream &ss, bool compact)
{
    ...
    for (Exports::const_iterator it = exports.begin(); it != exports.end(); ++it) {
        ...
        ss << std::setfill(' ') << std::setw(40) << std::left << name
                << " @0x" << std::hex << address << "\n";
    }
...
static void dumpImports(PEFile *peFile, std::ostream &ss, bool compact)
{
    ...
    if (imports.size() == 0) {
        if (!compact) {
            ss << "#The import directory is empty\n";
        }
        return;
    }
...
static void dumpRelocations(PEFile *peFile, std::ostream &ss, bool compact)
{
    ...
    if (relocations.size() == 0) {
        if (!compact) {
            ss << "#There are no relocations present\n";
        }
        return;
    }

    for (vmi::Relocations::const_iterator it = relocations.begin();
         it != relocations.end(); ++it) {
...
{
    fprintf(stderr, "Usage: %s [options] pe_file\n", progName);
    fprintf(stderr,
            "Options:\n\n"
            "  -h:  print header\n"
            "  -s:  print sections\n"
...
static void dumpPeFile(PEFile *peFile,
                       bool printHeader,
                       bool printExports,
                       bool printImports,
                       bool printSections,
...
int main(int argc, char **argv)
{
    ...
    if (peFile) {
        dumpPeFile(peFile, printHeader, printExports, printImports, printSections, printRelocations, printExceptions, compact);
    } else {
