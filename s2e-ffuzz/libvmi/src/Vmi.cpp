...
bool Vmi::allocateBufferAndFetchData(void *opaque, uint64_t address, const VmiType *type, void **buffer, unsigned *bufferSize) const
{
...
bool Vmi::dereferencePointer(const VmiType *type, uint64_t pointer, uint64_t *value, void *opaque)
{
    ...
    switch(pointerType->getSize()) {
        case 4: *value = *(uint32_t*) nextAddressBuf; break;
        case 8: *value = *(uint64_t*) nextAddressBuf; break;
        default: assert(false && "Unsupported pointer size");
    }
...
bool Vmi::getTypeAndAddress(const std::string &topLevelType, const std::string &path,
                            ...
                            void *opaque)
{
    ...
    if (!structType) {
        //If the top level is not a structure, it does not make sense to have a non-empty path
        if (path.size() != 0) {
            return false;
        }
...
bool Vmi::getData(const std::string &topLevelType, const std::string &path,
...
bool Vmi::getString(const std::string &topLevelType, const std::string &path,
...
bool Vmi::dump(llvm::raw_ostream &os, const std::string &strucName, uint64_t address, void *opaque, unsigned margin) const
{
