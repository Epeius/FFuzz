...
private:
    ...
    bool writeMemoryData(HEADER *Header);

    template <typename HEADER, typename CONTEXT>
    bool writeHeader(HEADER *Header, const CONTEXT *context,
                     const BugCheckDescription &bugDesc);
public:

    WindowsCrashDumpGenerator(
            FileProvider *virtualMemory,
            FileProvider *physicalMemory,
    ...
    /* Windows XP */
    bool generate(uint64_t pKdDebuggerDataBlock,
                  uint64_t pKpcrb,
    ...
    /* For newer OSes */
    bool generate(const BugCheckDescription &bugDesc,
