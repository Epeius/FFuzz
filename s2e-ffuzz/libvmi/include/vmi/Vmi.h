...
enum VmiTypeEnum {
    PRIMITIVE, POINTER, STRUCTURE, TYPEDEF,
    ARRAY, INVALID
};
...
public:
    static VmiPrimitiveType *get(unsigned size) {
        ...
        if (it == s_primitiveTypes.end()) {
            ...
            switch (ret->m_size) {
                case 1: ret->m_name = "char"; break;
                case 2: ret->m_name = "short"; break;
                case 4: ret->m_name = "int"; break;
                case 8: ret->m_name = "long"; break;
                default: ret->m_name = "unknown"; break;
            }
...
public:
    ...
    static VmiStructureType *build(const std::string &name,
...
private:
    ...
    bool allocateBufferAndFetchData(void *opaque, uint64_t address, const VmiType *type, void **buffer, unsigned *bufferSize) const;
...
public:
    ...
    bool dump(llvm::raw_ostream &os, const std::string &strucName, uint64_t address, void *opaque, unsigned margin) const;
    ...
    bool getTypeAndAddress(const std::string &topLevelType, const std::string &path,
                                uint64_t startAddress,
                                const VmiType **effectiveType, uint64_t *effectiveAddress,
                                void *opaque);
     ...
     */
    bool getData(const std::string &topLevelType, const std::string &path,
                 uint64_t address, void **data, unsigned *size, void *opaque);

    template <typename T>
    bool get(const std::string &topLevelType, const std::string &path,
                 uint64_t address, T *data, void *opaque)
    {
        T *retdata;
        uint32_t size;
    ...
    template <typename T>
    bool get(const VmiType *type,
                 uint64_t address, T *data, void *opaque) const
    {
        void *retdata;
        uint32_t size;
    ...
    bool getPrimitiveData(const VmiType *type,
                          uint64_t address, uint64_t *data, void *opaque) const
    {
        ...
        if (!primitiveType) {
         ...
         switch(type->getSize()) {
            case 1: return get(primitiveType, address, (uint8_t*)data, opaque);
            case 2: return get(primitiveType, address, (uint16_t*)data, opaque);
            case 4: return get(primitiveType, address, (uint32_t*)data, opaque);
            case 8: return get(primitiveType, address, (uint64_t*)data, opaque);
         }

         return false;
    }

    bool getString(const std::string &topLevelType, const std::string &path,
                        uint64_t address, std::string &result, void *opaque);
};
