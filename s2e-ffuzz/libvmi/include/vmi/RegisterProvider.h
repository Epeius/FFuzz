...
public:
    ...
    template <typename T>
    bool read(unsigned reg, T *value) {
        if (!m_read)
            return false;
    ...
    template <typename T>
    bool write(unsigned reg, const T *value) {
        if (!m_write)
            return false;
...
/* Prefix the regs to avoid conflicts. Ordering is important (clients may assume it) */
enum X86Registers {
    X86_EAX, X86_ECX, X86_EDX, X86_EBX, X86_ESP, X86_EBP, X86_ESI, X86_EDI,
    X86_ES, X86_CS, X86_SS, X86_DS, X86_FS, X86_GS,
...
public:
    X86RegisterProvider(void *opaque, ReadRegCb _read, WriteRegCb _write):
            RegisterProvider(opaque, _read, _write) {

    }

    template <typename T>
    bool read(X86Registers reg, T *value) {
        if (!m_read)
            return false;
        return m_read(m_opaque, reg, value, sizeof(*value));
    }
    ...
    template <typename T>
    bool write(X86Registers reg, const T *value) {
        if (!m_write)
            return false;
        return m_write(m_opaque, reg, value, sizeof(*value));
    }
