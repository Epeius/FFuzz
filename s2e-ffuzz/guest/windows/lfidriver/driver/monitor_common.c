...
/* These structure is shared with WindowsMonitor2 */
S2E_WINMON2_KERNEL_STRUCTS g_KernelStructs;

/**
 * This is for internal use by lfidriver.
 * It is not used by WindowsMonitor2.
 */
LFIDRIVER_KERNEL_STRUCTS g_LfiKernelStructs;
...
static void S2ERegisterModule(const AUX_MODULE_EXTENDED_INFO *Info, UCHAR* BaseName)
{
    ...
    S2EMessageFmt("Module Name:%s (%s) LoadBase:%#p Size:%#x\n",
        Info->FullPathName,
        BaseName,
...
NTSTATUS RegisterKernelStructures(PVOID KernelBase)
{
    ...
    if (DosHeader->e_magic != IMAGE_DOS_SIGNATURE) {
        S2EMessageFmt("Incorrect image DOS signature\n");
    ...
    if (NtHeaders->Signature != IMAGE_NT_SIGNATURE) {
        S2EMessageFmt("Incorrect image NT signature\n");
        return STATUS_INVALID_IMAGE_FORMAT;
    }

    S2EMessageFmt("Kernel image checksum: %#x\n", NtHeaders->OptionalHeader.CheckSum);
...
NTSTATUS RegisterLoadedModules()
{
    ...
    for (i = 0; i < BufferSize / sizeof(AUX_MODULE_EXTENDED_INFO); ++i) {
        ...
        if (strstr((const char*)BaseName, "ntoskrnl") || strstr((const char*)BaseName, "ntkrnlpa")) {
            try {
                ...
                S2EMessageFmt("Exception while parsing %s\n", BaseName);
            }
        }
    ...
    if (!KernelDataLoaded) {
        S2EKillState(-1, "Could not load kernel data info");
...
{
    S2EMessage("InitializeWindowsMonitor2\n");
